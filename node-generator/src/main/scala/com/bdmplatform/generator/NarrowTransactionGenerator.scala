...
import com.bdmplatform.generator.NarrowTransactionGenerator.{ScriptSettings, Settings}
import com.bdmplatform.generator.utils.{Gen, Universe}
...
import com.bdmplatform.state.DataEntry.{MaxValueSize, Type}
...
import com.bdmplatform.transaction.lease.{LeaseCancelTransaction, LeaseCancelTransactionV2, LeaseTransactionV2}
  ...
  private[this] object preconditions {
    ...
    val tradeAssetIssue = IssueTransactionV2
      .selfSigned(
        ...
        "Bdm DEX is the best exchange ever".getBytes("UTF-8"),
        100000000,
        2,
        reissuable = true,
        fee = 100400000L,
        ...
        val tx = typeGen.getRandom match {
          ...
          case ExchangeTransactionV1 =>
            (
              for {
                ...
                pair  = AssetPair(Bdm, IssuedAsset(preconditions.tradeAssetIssue.id()))
          ...
          case InvokeScriptTransaction =>
            ...
            val asset = randomFrom(Universe.IssuedAssets.filter(a => script.paymentAssets.contains(new String(a.name))))
              .fold(Bdm: Asset)(tx => IssuedAsset(tx.id()))
