...
import com.bdmplatform.generator.GeneratorSettings.NodeAddress
import com.bdmplatform.generator.Preconditions.{PGenSettings, UniverseHolder}
import com.bdmplatform.generator.cli.ScoptImplicits
import com.bdmplatform.generator.config.FicusImplicits
import com.bdmplatform.generator.utils.Universe
import com.bdmplatform.network.client.NetworkSender
...
import com.bdmplatform.utils.{LoggerFacade, NTP}
...
object TransactionsGeneratorApp extends App with ScoptImplicits with FicusImplicits with EnumerationReader {
  ...
  // IDEA bugs
  implicit val inetSocketAddressReader: ValueReader[InetSocketAddress] = com.bdmplatform.settings.inetSocketAddressReader
  ...
  val parser = new OptionParser[GeneratorSettings]("generator") {
    head("TransactionsGenerator - Bdm load testing transactions generator")
  ...
  val bdmSettings = BdmSettings.fromRootConfig(ConfigFactory.load())
  ...
  parser.parse(args, defaultConfig) match {
    ...
    case Some(finalConfig) =>
      ...
      val estimator = bdmSettings.estimator
      ...
      val sender = new NetworkSender(bdmSettings.networkSettings.trafficLogger, finalConfig.addressScheme, "generator", nonce = Random.nextLong())
