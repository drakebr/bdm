...
import com.bdmplatform.generator.utils.Implicits._
...
object Gen {
  ...
  def transfers(senderGen: Iterator[KeyPair], recipientGen: Iterator[Address], feeGen: Iterator[Long]): Iterator[Transaction] = {
    ...
    senderGen
      ...
      .map {
        case (((src, dst), fee), i) =>
          TransferTransactionV2.selfSigned(Bdm, src, dst, fee, now + i, Bdm, fee, Array.emptyByteArray)
      }
    ...
    senderGen
      ...
      .map {
        case ((sender, count), i) =>
          ...
          MassTransferTransaction.selfSigned(Bdm, sender, transfers, now + i, fee, Array.emptyByteArray)
      }
