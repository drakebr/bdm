...
import com.bdmplatform.account.{Address, AddressScheme, KeyPair}
...
import com.bdmplatform.transaction.assets.{IssueTransaction, IssueTransactionV2}
import com.bdmplatform.transaction.lease.{LeaseTransaction, LeaseTransactionV2}
          ...
          action match {
            ...
            case CreateAccountP(seed, balance, scriptOption) =>
              ...
              val transferTx = TransferTransactionV2
                .selfSigned(Bdm, settings.faucet, acc, balance, time.correctedTime(), Bdm, Fee, "Generator".getBytes("UTF-8"))
                .explicitGet()
