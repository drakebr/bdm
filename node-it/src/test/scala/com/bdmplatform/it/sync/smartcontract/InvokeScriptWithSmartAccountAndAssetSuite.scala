...
import com.bdmplatform.it.sync.{issueFee, minFee, smartFee, smartMinFee}
...
class InvokeScriptWithSmartAccountAndAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_send bdm to dApp and caller accounts") {
    val dAppTransferId        = sender.transfer(sender.address, dApp.stringRepr, 5.bdm, minFee).id
    val callerTransferId      = sender.transfer(sender.address, caller.stringRepr, 5.bdm, minFee).id
    val smartCallerTransferId = sender.transfer(sender.address, smartCaller.stringRepr, 5.bdm, minFee).id
  ...
  test("invoke by smart account requires just 1 extra fee") {
    assertBadRequestAndMessage(
      ...
      s"does not exceed minimal value of 900000 BDM"
    )
  }

  test("max fee is 0.053 Bdm (0.005 + extraFee(1 smart caller + 1 payment + 10 transfers))") {
  ...
  test("can't invoke with small fee for both smart assets") {
    ...
    assertBadRequestAndMessage(
      sender
        ...
        ._1.id,
      "does not exceed minimal value of 900000 BDM"
    )
  }
