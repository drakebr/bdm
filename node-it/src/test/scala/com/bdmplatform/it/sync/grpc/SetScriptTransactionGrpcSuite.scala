...
import com.bdmplatform.api.grpc.TransactionsApiGrpc
...
import com.bdmplatform.common.utils.{Base64, EitherExt2}
import com.bdmplatform.lang.script.Script
import com.bdmplatform.protobuf.Amount
import com.bdmplatform.protobuf.transaction.{PBTransactions, Recipient, SetScriptTransactionData, SignedTransaction, TransferTransactionData, Script => PBScript, Transaction => PBTransaction}
...
class SetScriptTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("able to broadcast tx if that is allowed by account-script") {
    ...
    val thirdBalance = sender.grpc.bdmBalance(thirdAddress).available
    ...
    sender.grpc.bdmBalance(firstAddress).available shouldBe firstBalance - transferAmount - transferFee
    sender.grpc.bdmBalance(thirdAddress).available shouldBe thirdBalance + transferAmount
  }
  ...
  test("able to clear script from scripted account") {
    ...
    sender.grpc.bdmBalance(firstAddress).available shouldBe firstBalance - transferAmount - minFee
    sender.grpc.bdmBalance(secondAddress).available shouldBe secondBalance + transferAmount
  }
  ...
  test("not able to broadcast tx from scripted acc if tx fee doesn't include smart fee") {
    ...
    sender.grpc.bdmBalance(thirdAddress).available shouldBe thirdBalance
