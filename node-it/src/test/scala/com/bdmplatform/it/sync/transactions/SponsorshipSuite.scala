...
import com.bdmplatform.it.{NodeConfigs, ReportingTestName}
import com.bdmplatform.state.diffs.FeeValidation
import com.bdmplatform.transaction.Asset.IssuedAsset
import com.bdmplatform.transaction.assets.SponsorFeeTransaction
...
class SponsorshipSuite extends FreeSpec with NodesFromDocker with Matchers with ReportingTestName with CancelAfterFailure {
  ...
  override def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("bdm.blockchain.custom.functionality.blocks-for-feature-activation=1"))
      .overrideBase(_.raw("bdm.blockchain.custom.functionality.feature-check-blocks-period=1"))
      .withDefault(1)
  ...
  "Fee in sponsored asset works fine" - {
    ...
    val sponsorBdmBalance = sponsor.accountBalances(sponsor.address)._2
    val minerBdmBalance   = miner.accountBalances(miner.address)._2
    ...
    "check balance before test accounts balances" in {
      ...
      sponsor.assertBalances(sponsor.address, sponsorBdmBalance - 2 * issueFee - minFee)
    ...
    val minerBdmBalanceAfterFirstXferTest   = minerBdmBalance + 2.bdm + minFee + FeeValidation.FeeUnit * SmallFee / minSponsorFee
    val sponsorBdmBalanceAfterFirstXferTest = sponsorBdmBalance - 2.bdm - minFee - FeeValidation.FeeUnit * SmallFee / minSponsorFee
...
//        sponsorTx.head.count(tx => tx.sender.contains(sponsor.address) || tx.recipient.contains(sponsor.address)) shouldBe 4
      ...
      "sponsor should receive sponsored asset as fee, bdm should be written off" in {
        miner.assertAssetBalance(sponsor.address, sponsorAssetId, sponsorAssetTotal / 2 + SmallFee)
        miner.assertBalances(sponsor.address, sponsorBdmBalanceAfterFirstXferTest)
      }

      "miner bdm balance should be changed" in {
        miner.assertBalances(miner.address, minerBdmBalanceAfterFirstXferTest)
      }
    }

    "assets balance should contain sponsor fee info and sponsor balance" in {
      val sponsorLeaseSomeBdm = sponsor.lease(sponsor.address, bob.address, leasingAmount, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseSomeBdm)
    ...
    "bdm fee depends on sponsor fee and sponsored token decimals" in {
      val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.bdm, LargeFee, None, Some(sponsorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
      assert(!transferTxCustomFeeAlice.isEmpty)
      ...
      miner.assertBalances(
        sponsor.address,
        sponsorBdmBalanceAfterFirstXferTest - FeeValidation.FeeUnit * LargeFee / Token - leasingFee,
        sponsorBdmBalanceAfterFirstXferTest - FeeValidation.FeeUnit * LargeFee / Token - leasingFee - leasingAmount
      )
      miner.assertBalances(miner.address, minerBdmBalanceAfterFirstXferTest + FeeValidation.FeeUnit * LargeFee / Token + leasingFee)
    }
    ...
    "cancel sponsorship" - {
      ...
      "check sponsor and miner balances after cancel" in {
        miner.assertBalances(
          sponsor.address,
          sponsorBdmBalanceAfterFirstXferTest - FeeValidation.FeeUnit * LargeFee / Token - leasingFee - issueFee,
          sponsorBdmBalanceAfterFirstXferTest - FeeValidation.FeeUnit * LargeFee / Token - leasingFee - leasingAmount - issueFee
        )
        miner.assertBalances(miner.address, minerBdmBalanceAfterFirstXferTest + FeeValidation.FeeUnit * LargeFee / Token + leasingFee + issueFee)
      }
    ...
    "set sponsopship again" - {
      ...
      "make transfer with new min sponsored fee" in {
        ...
        val aliceBdmBalance   = alice.accountBalances(alice.address)
        val bobAssetBalance     = bob.assetBalance(bob.address, sponsorAssetId).balance
        val bobBdmBalance     = bob.accountBalances(bob.address)
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.bdm, TinyFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        nodes.waitForHeightArise()

        val bdmFee = FeeValidation.FeeUnit * TinyFee / TinyFee
        sponsor.assertBalances(sponsor.address, sponsoredBalance._1 - bdmFee, sponsoredBalance._2 - bdmFee)
        ...
        alice.assertBalances(alice.address, aliceBdmBalance._2 - 1.bdm)
        bob.assertBalances(bob.address, bobBdmBalance._1 + 1.bdm, bobBdmBalance._2 + 1.bdm)
        bob.assertAssetBalance(bob.address, sponsorAssetId, bobAssetBalance)
        miner.assertBalances(miner.address, minerBalance._2 + bdmFee)
      ...
      "transfer tx sponsored fee is less then new minimal" in {
        assertBadRequestAndResponse(
          sponsor
            ...
            .id,
          s"Fee for TransferTransaction \\($SmallFee in ${Some(sponsorAssetId).get}\\) does not exceed minimal value of 100000 BDM or $LargeFee ${Some(sponsorAssetId).get}"
        )
      }
      ...
      "make transfer with updated min sponsored fee" in {
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.bdm, LargeFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        val bdmFee = FeeValidation.FeeUnit * LargeFee / LargeFee
        nodes.waitForHeightArise()
    ...
    "issue asset make sponsor and burn and reissue" in {
      ...
      val aliceTransferBdm = alice.transfer(alice.address, bob.address, transferAmount, SmallFee, None, Some(sponsorAssetId2)).id
      nodes.waitForHeightAriseAndTxPresent(aliceTransferBdm)
      nodes.waitForHeightArise()

      val totalBdmFee = FeeValidation.FeeUnit * SmallFee / Token + issueFee + sponsorFee + burnFee + minFee + issueFee
      miner.assertBalances(miner.address, minerBalance._1 + totalBdmFee)
      sponsor.assertBalances(sponsor.address, sponsorBalance._1 - totalBdmFee, sponsorBalance._2 - totalBdmFee)
    ...
    "miner is sponsor" in {
      ...
      val aliceSponsoredTransferBdm = alice.transfer(alice.address, bob.address, transferAmount, SmallFee, None, Some(minersSpondorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(aliceSponsoredTransferBdm)
      nodes.waitForHeightArise()
    ...
    "tx is declined if sponsor has not enough effective balance to pay fee" in {
      ...
      val sponsorLeaseAllAvaliableBdm             = sponsor.lease(sponsor.address, bob.address, sponsorEffectiveBalance - leasingFee, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseAllAvaliableBdm)
      assertBadRequestAndMessage(alice.transfer(alice.address, bob.address, 10 * Token, LargeFee, Some(sponsorAssetId), Some(sponsorAssetId)),
                                 "unavailable funds")
      val cancelLeasingTx = sponsor.cancelLease(sponsor.address, sponsorLeaseAllAvaliableBdm, leasingFee).id
