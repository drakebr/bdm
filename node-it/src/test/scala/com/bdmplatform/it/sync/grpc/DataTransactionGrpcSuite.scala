...
import com.bdmplatform.protobuf.transaction.DataTransactionData.DataEntry
import com.bdmplatform.protobuf.transaction.PBTransactions
import com.bdmplatform.state.{DataEntry => VanillaDataEntry}
import com.bdmplatform.state.DataEntry.{MaxKeySize, MaxValueSize}
import com.bdmplatform.transaction.DataTransaction.MaxEntryCount
import com.bdmplatform.transaction.TxValidationError.TooBigArray
import io.grpc.Status.Code

import scala.concurrent.duration._

class DataTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  test("sender's bdm balance is decreased by fee.") {
    val firstBalance = sender.grpc.bdmBalance(firstAddress).available
    val firstEffBalance = sender.grpc.bdmBalance(firstAddress).effective
    ...
    sender.grpc.bdmBalance(firstAddress).available shouldBe firstBalance - fee
    sender.grpc.bdmBalance(firstAddress).effective shouldBe firstEffBalance - fee
  }

  test("cannot put data without having enough bdm") {
    val firstBalance = sender.grpc.bdmBalance(firstAddress).available
    val firstEffBalance = sender.grpc.bdmBalance(firstAddress).effective
    ...
    sender.grpc.bdmBalance(firstAddress).available shouldBe firstBalance
    sender.grpc.bdmBalance(firstAddress).effective shouldBe firstEffBalance
  }
  ...
  test("data definition and retrieval") {
    ...
    val firstBalance = sender.grpc.bdmBalance(secondAddress).available
    val firstEffBalance = sender.grpc.bdmBalance(secondAddress).effective
    ...
    sender.grpc.bdmBalance(secondAddress).available shouldBe firstBalance - fee
    sender.grpc.bdmBalance(secondAddress).effective shouldBe firstEffBalance - fee
  }
  ...
  test("try to send tx above limits of key, value and size") {
    ...
    assertGrpcError(sender.grpc.putData(firstAcc, List(DataEntry("", DataEntry.Value.BoolValue(false))), 1.bdm), "Empty key found", Code.INTERNAL)
    assertGrpcError(
      sender.grpc.putData(firstAcc, List(DataEntry("abc", DataEntry.Value.BoolValue(false)), DataEntry("abc", DataEntry.Value.BoolValue(false))), 1.bdm),
