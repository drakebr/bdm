...
import com.bdmplatform.state.Sponsorship
...
class CustomFeeTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("make transfer with sponsored asset") {
    ...
    val sponsorship = Sponsorship.toBdm(transferFee, assetToken)
...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""
                                                         | bdm.fees.transfer.$assetId = 100000
                                                         | bdm.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("bdm.miner.enable=no").withFallback(minerConfig)
