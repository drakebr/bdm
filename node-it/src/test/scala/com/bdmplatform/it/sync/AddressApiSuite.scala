...
import com.bdmplatform.api.http.ApiError.{CustomValidationError, TooBigArrayAllocation}
import com.bdmplatform.it.api.SyncHttpApi._
import com.bdmplatform.it.transactions.BaseTransactionSuite
import com.bdmplatform.it.{NTPTime, NodeConfigs}
...
class AddressApiSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("balances for bdm should be correct") {
    assertBalances(None)
  }
  ...
  test("limit violation requests should be handled correctly") {
    val limit     = miner.config.getInt("bdm.rest-api.transactions-by-address-limit")
  ...
  override protected def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("bdm.rest-api.transactions-by-address-limit = 20"))
      .withDefault(1)
