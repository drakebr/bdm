...
import com.bdmplatform.it.api.{DataResponse, DebugStateChanges, StateChangesDetails, TransactionInfo, TransfersInfoResponse}
import com.bdmplatform.it.sync.setScriptFee
import com.bdmplatform.it.transactions.BaseTransactionSuite
import com.bdmplatform.it.util._
import com.bdmplatform.lang.v1.compiler.Terms.{CONST_LONG, CONST_STRING}
...
class InvokeScriptTransactionStateChangesSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("prepare") {
    ...
    sender.massTransfer(contract.stringRepr, List(Transfer(caller.stringRepr, 3000), Transfer(recipient.stringRepr, 3000)), 0.01.bdm, Some(simpleAsset))
    ...
    val script = ScriptCompiler.compile("""
        ...
        |
        |@Callable(i)
        |func writeAndSendBdm(value: Int, recipient: String, amount: Int) = {
        |    ScriptResult(
        |        WriteSet([DataEntry("result", value)]),
