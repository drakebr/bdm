...
import com.bdmplatform.api.http.ApiError.{CustomValidationError, Mistiming, StateCheckFailed}
...
import com.bdmplatform.it.sync.{script, someAssetAmount, _}
...
import com.bdmplatform.transaction.assets.SetAssetScriptTransaction
...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("cannot transact without having enough bdm") {
    ...
    assertApiError(sender.setAssetScript(assetWScript, firstAddress, balance + 1, Some(scriptBase64))) { error =>
      error.id shouldBe StateCheckFailed.Id
      error.message shouldBe StateCheckFailed.message(s"negative bdm balance: $firstAddress, old: $balance, new: -1")
    }
