...
import com.bdmplatform.it.api.UnexpectedStatusCodeException
import com.bdmplatform.it.sync.{calcDataFee, minFee}
import com.bdmplatform.it.transactions.BaseTransactionSuite
import com.bdmplatform.it.util._
import com.bdmplatform.state.{BinaryDataEntry, BooleanDataEntry, DataEntry, IntegerDataEntry, StringDataEntry}
...
class DataTransactionSuite extends BaseTransactionSuite {
  ...
  test("cannot transact without having enough bdm") {
    val (balance1, eff1) = miner.accountBalances(firstAddress)

    val data = List(BooleanDataEntry("bool", false))
    assertBadRequestAndResponse(sender.putData(firstAddress, data, balance1 + 1), "negative bdm balance")
    nodes.waitForHeightArise()
    miner.assertBalances(firstAddress, balance1, eff1)

    val leaseAmount = 1.bdm
  ...
  test("try to send tx above limits of key, value and size") {
    ...
    assertBadRequestAndResponse(sender.putData(firstAddress, extraValueData, 1.bdm), TooBig)
    nodes.waitForHeightArise()

    val largeBinData = List.tabulate(5)(n => BinaryDataEntry(extraKey, ByteStr(Array.fill(MaxValueSize)(n.toByte))))
    assertBadRequestAndResponse(sender.putData(firstAddress, largeBinData, 1.bdm), TooBig)
    nodes.waitForHeightArise()

    val largeStrData = List.tabulate(5)(n => StringDataEntry(extraKey, "A" * MaxValueSize))
    assertBadRequestAndResponse(sender.putData(firstAddress, largeStrData, 1.bdm), TooBig)
    nodes.waitForHeightArise()

    val tooManyEntriesData = List.tabulate(MaxEntryCount + 1)(n => IntegerDataEntry("key", 88))
    assertBadRequestAndResponse(sender.putData(firstAddress, tooManyEntriesData, 1.bdm), TooBig)
    nodes.waitForHeightArise()
  }
