...
import com.bdmplatform.it._
import com.bdmplatform.it.api.AsyncHttpApi._
import com.bdmplatform.it.api._
import com.bdmplatform.it.transactions.BaseTransactionSuiteLike
  ...
  protected override def beforeAll(): Unit = {
    ...
    val fundAndIssue =
      for {
        ...
        fundTx <- node.transfer(node.address, issuer.stringRepr, 1000.bdm, 0.001.bdm)
    ...
    val transactions =
      Future.sequence(addrs map { addr =>
        NodeAsyncHttpApi(faucet)
          .transfer(faucet.address, addr, 1000.bdm, 0.001.bdm)
          .flatMap { tx =>
            NodeAsyncHttpApi(faucet)
