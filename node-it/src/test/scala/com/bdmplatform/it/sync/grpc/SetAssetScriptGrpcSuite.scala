...
class SetAssetScriptGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("sender's bdm balance is decreased by fee") {
    ...
    sender.grpc.bdmBalance(firstAddress).available shouldBe firstBalance - setAssetScriptFee
    sender.grpc.bdmBalance(firstAddress).effective shouldBe firstEffBalance - setAssetScriptFee
  }

  test("not able set script without having enough bdm") {
    val firstBalance     = sender.grpc.bdmBalance(firstAddress).available
    val firstEffBalance  = sender.grpc.bdmBalance(firstAddress).effective
    assertGrpcError(
  ...
  test("try to make SetAssetScript for asset v1") {
    ...
    val balance     = sender.grpc.bdmBalance(thirdAddress).available
    val effBalance  = sender.grpc.bdmBalance(thirdAddress).effective
    ...
    sender.grpc.bdmBalance(thirdAddress).available shouldBe balance
    sender.grpc.bdmBalance(thirdAddress).effective shouldBe effBalance
