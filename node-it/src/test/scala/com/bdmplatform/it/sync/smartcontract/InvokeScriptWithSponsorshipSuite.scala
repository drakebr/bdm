...
import com.bdmplatform.it.sync.{minFee, smartMinFee}
...
class InvokeScriptWithSponsorshipSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("required fee in sponsored assets considers scripts count") {
    ...
    assertBadRequestAndMessage(
      ...
      s"does not exceed minimal value of 900000 BDM or $feeAmount"
    )
    assertBadRequestAndMessage(
      sender.invokeScript(
        caller.stringRepr,
      ...
      s"does not exceed minimal value of 5300000 BDM"
    )
    ...
    sender.accountBalances(dApp.stringRepr)._1 shouldBe dAppInitBalance - 0.009.bdm - 0.053.bdm
