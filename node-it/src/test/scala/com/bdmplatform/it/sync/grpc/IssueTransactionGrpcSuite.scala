...
import com.bdmplatform.transaction.TxValidationError.{InvalidName, NonPositiveAmount, TooBigArray}
...
class IssueTransactionGrpcSuite extends GrpcBaseTransactionSuite with NTPTime with TableDrivenPropertyChecks {
  ...
  test("asset issue changes issuer's asset balance") {
    for (v <- supportedVersions) {
      ...
      val issuerBalance = sender.grpc.bdmBalance(issuerAddress).available
      val issuerEffBalance = sender.grpc.bdmBalance(issuerAddress).effective
      ...
      sender.grpc.bdmBalance(issuerAddress).available shouldBe issuerBalance - issueFee
      sender.grpc.bdmBalance(issuerAddress).effective shouldBe issuerEffBalance - issueFee
  ...
  test("not able to issue asset with fee less then issueFee (minFee for NFT)") {
    for (v <- supportedVersions) {
      ...
      sender.grpc.bdmBalance(issuerAddress).available shouldBe issuerBalance
      sender.grpc.bdmBalance(issuerAddress).effective shouldBe issuerEffBalance
    }
  }
  ...
  test("Not able to create asset when insufficient funds") {
    ...
    val issuerEffBalance = sender.grpc.bdmBalance(issuerAddress).effective
    val bigAssetFee      = issuerEffBalance + 1.bdm
