...
class BurnTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("burning assets changes issuer's asset balance; issuer's bdm balance is decreased by fee") {
    for (v <- supportedVersions) {
      ...
      val balance = sender.grpc.bdmBalance(firstAddress)
      sender.grpc.broadcastBurn(firstAcc, issuedAssetId, issueAmount / 2, minFee, version = v, waitForTx = true)

      sender.grpc.bdmBalance(firstAddress).available shouldBe balance.available - minFee
      sender.grpc.bdmBalance(firstAddress).effective shouldBe balance.effective - minFee
