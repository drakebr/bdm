...
class LeasingTransactionsSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("can not make leasing without having enough balance") {
    for (v <- supportedVersions) {
      ...
      assertBadRequestAndResponse(sender.lease(secondAddress, firstAddress, balance2 + 1.bdm, minFee, version = v), errorMessage)
  ...
  test("lease cancellation reverts eff.b. changes; lessor pays fee for both lease and cancellation") {
    import com.bdmplatform.transaction.lease.LeaseTransaction.Status._
  ...
  test("can not make leasing to yourself") {
    for (v <- supportedVersions) {
      val (balance1, eff1) = miner.accountBalances(firstAddress)
      assertBadRequestAndResponse(sender.lease(firstAddress, firstAddress, balance1 + 1.bdm, minFee, v), "Transaction to yourself")
      nodes.waitForHeightArise()
