...
import com.bdmplatform.it.sync.{issueFee, minFee, scriptBase64, setScriptFee, someAssetAmount}
...
class InvokeScriptErrorMsgSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.transfer(sender.address, recipient = contract.stringRepr, assetId = None, amount = 5.bdm, fee = minFee, waitForTx = true).id
    sender.transfer(sender.address, recipient = contract.stringRepr, assetId = None, amount = 5.bdm, fee = minFee, waitForTx = true).id
  ...
  test("error message is informative") {
    ...
    assertBadRequestAndMessage(
      ...
      "State check failed. Reason: Transaction sent from smart account. Requires 400000 extra fee. Transaction involves 1 scripted assets." +
        " Requires 400000 extra fee. Fee for InvokeScriptTransaction (1000 in BDM) does not exceed minimal value of 1300000 BDM."
    )
    ...
    assertBadRequestAndMessage(
      ...
      "State check failed. Reason: Fee in BDM for InvokeScriptTransaction (1300000 in BDM) with 12 total scripts invoked does not exceed minimal value of 5300000 BDM."
    )
  }
}
