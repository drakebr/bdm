...
class UtxSuite extends FunSuite with CancelAfterFailure with NodesFromDocker with Matchers {
  ...
  test("Invalid transaction should be removed from from utx") {
    ...
    val transferToAccount = TransferTransactionV1
      .selfSigned(Bdm, miner.privateKey, account, AMOUNT, System.currentTimeMillis(), Bdm, ENOUGH_FEE, Array.emptyByteArray)
      .explicitGet()
    ...
    val firstTransfer = TransferTransactionV1
      .selfSigned(Bdm, account, miner.privateKey, AMOUNT - ENOUGH_FEE, System.currentTimeMillis(), Bdm, ENOUGH_FEE, Array.emptyByteArray)
      .explicitGet()

    val secondTransfer = TransferTransactionV1
      .selfSigned(Bdm, account, notMiner.privateKey, AMOUNT - ENOUGH_FEE, System.currentTimeMillis(), Bdm, ENOUGH_FEE, Array.emptyByteArray)
      .explicitGet()
