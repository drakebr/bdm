...
import com.bdmplatform.transaction.lease.LeaseTransaction.Status.{Active, Canceled}
...
object LeaseStatusTestSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""bdm {
       |   miner{
       |      enable = yes
  ...
  private val notMinerConfig = ConfigFactory.parseString(s"""bdm {
       |   miner.enable = no
