package com.bdmplatform.it.sync

import com.typesafe.config.{Config, ConfigFactory}
import com.bdmplatform.api.http.ApiError.CustomValidationError
import com.bdmplatform.features.{BlockchainFeatureStatus, BlockchainFeatures}
import com.bdmplatform.it.NodeConfigs.Default
import com.bdmplatform.it.ReportingTestName
import com.bdmplatform.it.api.SyncHttpApi._
import com.bdmplatform.it.sync.activation.ActivationStatusRequest
...
class RewardsTestSuite
  ...
  "reward changes accordingly node's votes and miner's balance changes by reward amount after block generation" - {
    ...
    val initialAmount = BigInt(Constants.TotalBdm) * BigInt(Constants.UnitsInWave)
    ...
    "when miner votes for increase" in {
      ...
      rewardAtActivation.totalBdmAmount shouldBe initialAmount + initial
      ...
      rewardAfterFirstVote.totalBdmAmount shouldBe initialAmount + BigInt(initial) * BigInt(votingStartHeight - activationHeight + 1)
      ...
      rewardAtTermEnd.totalBdmAmount shouldBe amountAfterTerm
      ...
      rewardSecVoting.totalBdmAmount shouldBe amountAfterTerm + newReward * BigInt(secondVotingStartHeightPlusTwo - termEndHeight)
    }
