...
import com.bdmplatform.it.sync.smartcontract.{cryptoContextScript, pureContextScript, bdmContextScript, _}
...
class ExchangeSmartAssetsSuite extends BaseTransactionSuite with CancelAfterFailure with NTPTime {
  ...
  test("require using a certain matcher with smart accounts") {
    ...
    val smartPair = AssetPair(IssuedAsset(ByteStr.decodeBase58(sAsset).get), Bdm)
  ...
  test("AssetPair from smart assets") {
    ...
    withClue("check fee for smart accounts and smart AssetPair - extx.fee == 0.015.bdm") {
      ...
      assertBadRequestAndMessage(
        ...
        "does not exceed minimal value of 1500000 BDM"
      )
  ...
  test("use all functions from RIDE for asset script") {
    ...
    val script3 = Some(ScriptCompiler(bdmContextScript(dtx, false), isAssetScript = true, estimator).explicitGet()._1.bytes.value.base64)
    ...
    List(script1, script2, script3)
      .map { i =>
        ...
        val smartPair = AssetPair(IssuedAsset(ByteStr.decodeBase58(asset).get), Bdm)
