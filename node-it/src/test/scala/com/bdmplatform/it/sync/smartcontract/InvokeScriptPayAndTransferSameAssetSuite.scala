...
import com.bdmplatform.it.sync.{setScriptFee, smartFee, smartMinFee}
...
class InvokeScriptPayAndTransferSameAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_set script to dApp account and transfer out all bdm") {
    ...
    val dAppScript        = ScriptCompiler.compile(s"""
          ...
          |  else throw("need payment in BDM or any Asset")
          |}
  ...
  test("dApp can transfer payed Bdm if its own balance is 0") {
    dAppInitBalance = sender.accountBalances(dApp)._1
    callerInitBalance = sender.accountBalances(caller)._1
    receiverInitBalance = sender.accountBalances(receiver)._1
  ...
  def invoke(func: String, amount: Long, asset: Asset = Bdm, fee: Long = 500000): String = {
    sender
      .invokeScript(
        caller,
        dApp,
