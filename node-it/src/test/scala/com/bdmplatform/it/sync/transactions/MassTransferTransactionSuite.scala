...
import com.bdmplatform.api.http.assets.{MassTransferRequest, SignedMassTransferRequest}
...
import com.bdmplatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("can not make mass transfer without having enough bdm") {
    ...
    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative bdm balance")
  ...
  test("invalid transfer should not be in UTX or blockchain") {
    ...
    def request(transfers: List[Transfer] = List(Transfer(secondAddress, transferAmount)),
      ...
      val txEi = for {
        ...
        tx              <- MassTransferTransaction.selfSigned(Bdm, sender.privateKey, parsedTransfers, timestamp, fee, attachment)
      } yield tx
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.bdm), Transfer(secondAddress, 2.bdm), Transfer(thirdAddress, 3.bdm))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.bdm)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.bdm)
  ...
  test("reporting MassTransfer transactions to aliases") {
    ...
    val transfers = aliases.map { alias =>
      Transfer(Alias.create(alias).explicitGet().stringRepr, 2.bdm)
    }
    val txId = sender.massTransfer(firstAddress, transfers, 300000).id
    nodes.waitForHeightAriseAndTxPresent(txId)
