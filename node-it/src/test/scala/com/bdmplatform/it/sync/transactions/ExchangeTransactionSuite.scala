...
import com.bdmplatform.it.sync.smartcontract.exchangeTx
...
class ExchangeTransactionSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("cannot exchange non-issued assets") {
    for ((o1ver, o2ver, tver) <- versions) {
      ...
      val buyAmount           = 1
      val sellAmount          = 1
      pair = AssetPair.createAssetPair("BDM", assetId).get
  ...
  test("negative - check orders v2 and v3 with exchange tx v1") {
    ...
    pair = AssetPair.createAssetPair("BDM", exchAsset.id().base58).get
  ...
  test("exchange tx with orders v3") {
    ...
    for ((o1ver, o2ver, matcherFeeOrder1, matcherFeeOrder2) <- Seq(
           (1: Byte, 3: Byte, Bdm, IssuedAsset(assetId)),
           (1: Byte, 3: Byte, Bdm, Bdm),
           (2: Byte, 3: Byte, Bdm, IssuedAsset(assetId)),
           (3: Byte, 1: Byte, IssuedAsset(assetId), Bdm),
           (2: Byte, 3: Byte, Bdm, Bdm),
           (3: Byte, 2: Byte, IssuedAsset(assetId), Bdm)
         )) {
      ...
      val assetPair  = AssetPair.createAssetPair("BDM", assetId.base58).get
