...
import com.bdmplatform.it.api.AsyncHttpApi._
import com.bdmplatform.it.util.GlobalTimer.{instance => timer}
...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false, imageName: String = Docker.NodeImageName)
  ...
  private val logDir: Coeval[Path] = Coeval.evalOnce {
    val r = Option(System.getProperty("bdm.it.logging.dir"))
      .map(Paths.get(_))
  ...
  private lazy val bdmNetwork: Network = {
    val id          = Random.nextInt(Int.MaxValue)
    val networkName = s"bdm-$id"
  ...
  def createNetwork: Network = bdmNetwork
  ...
  private def startNodeInternal(nodeConfig: Config, autoConnect: Boolean = true): DockerNode =
    try {
      val nodeName = nodeConfig.getString("bdm.network.node-name")
      ...
      val networkPort = actualConfig.getString("bdm.network.port")
      ...
      val configOverrides: String = {
        val ntpServer    = Option(System.getenv("NTP_SERVER")).fold("")(x => s"-Dbdm.ntp-server=$x ")
        val maxCacheSize = Option(System.getenv("MAX_CACHE_SIZE")).fold("")(x => s"-Dbdm.max-cache-size=$x ")

        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dbdm.network.declared-address=$ip:$networkPort $ntpServer $maxCacheSize"
        ...
        if (enableProfiling) {
          ...
          config += s"-agentpath:/usr/local/YourKit-JavaProfiler-2019.8/bin/linux-x86-64/libyjpagent.so=port=$ProfilerPort,listen=all," +
            s"sampling,monitors,sessionname=BdmNode,dir=$ContainerRoot/profiler,logdir=$ContainerRoot,onexit=snapshot "
        }
      ...
      val containerConfig = ContainerConfig
        .builder()
        .image(imageName)
        .networkingConfig(ContainerConfig.NetworkingConfig.create(Map(bdmNetwork.name() -> endpointConfigFor(nodeName)).asJava))
        .hostConfig(hostConfig)
        .env(s"BDM_OPTS=$configOverrides")
        .build()

      val containerId = {
        val jenkinsJobIdFromEnv = sys.env.get("JENKINS_JOB_ID").fold("")(s => s"-$s")
        val containerName = s"${bdmNetwork.name()}-$nodeName$jenkinsJobIdFromEnv"
        dumpContainers(
      ...
      val node = new DockerNode(actualConfig, containerId, getNodeInfo(containerId, BdmSettings.fromRootConfig(actualConfig)))
      nodes.add(node)
  ...
  private def getNodeInfo(containerId: String, settings: BdmSettings): NodeInfo = {
    ...
    val bdmIpAddress = containerInfo.networkSettings().networks().get(bdmNetwork.name()).ipAddress()

    NodeInfo(restApiPort, networkPort, bdmIpAddress, containerInfo.networkSettings().ports())
  }

  private def inspectContainer(containerId: String): ContainerInfo = {
    val containerInfo = client.inspectContainer(containerId)
    if (containerInfo.networkSettings().networks().asScala.contains(bdmNetwork.name())) containerInfo
    else {
      log.debug(s"Container $containerId has not connected to the network ${bdmNetwork.name()} yet, retry")
      Thread.sleep(1000)
  ...
  def restartNode(node: DockerNode, configUpdates: Config = empty): DockerNode = {
    ...
    if (configUpdates != empty) {
      ...
      val shPath = "/opt/bdm/start-bdm.sh"
      val scriptCmd: Array[String] =
        Array("sh", "-c", s"sed -i 's|$$BDM_OPTS.*-cp|$$BDM_OPTS $renderedConfig -cp|' $shPath && chmod +x $shPath")
  ...
  override def close(): Unit = {
    if (isStopped.compareAndSet(false, true)) {
      ...
      try {
        client.removeNetwork(bdmNetwork.id)
      } catch {
        case NonFatal(e) =>
          // https://github.com/moby/moby/issues/17217
          log.warn(s"Can not remove network ${bdmNetwork.name()}", e)
      }
  ...
  private def disconnectFromNetwork(containerId: String): Unit = {
    ...
    client.disconnectFromNetwork(containerId, bdmNetwork.id())
  }
...
object Docker {
  val NodeImageName: String = "com.bdmplatform/node-it:latest"

  private val ContainerRoot = Paths.get("/opt/bdm")
  ...
  def genesisOverride: Config = {
    ...
    val timestampOverrides = parseString(s"""bdm.blockchain.custom.genesis {
                                            |  timestamp = $genesisTs
    ...
    val gs               = genesisConfig.as[GenesisSettings]("bdm.blockchain.custom.genesis")
    val genesisSignature = Block.genesis(gs).explicitGet().uniqueId

    parseString(s"bdm.blockchain.custom.genesis.signature = $genesisSignature").withFallback(timestampOverrides)
  }

  AddressScheme.current = new AddressScheme {
    override val chainId: Byte = configTemplate.as[String]("bdm.blockchain.custom.address-scheme-character").charAt(0).toByte
  }
  ...
  case class NodeInfo(restApiPort: Int, networkPort: Int, bdmIpAddress: String, ports: JMap[String, JList[PortBinding]]) {
    ...
    val containerNetworkAddress: InetSocketAddress = new InetSocketAddress(bdmIpAddress, networkPort)
