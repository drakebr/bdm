...
import com.bdmplatform.account.{AddressOrAlias, AddressScheme, KeyPair}
import com.bdmplatform.api.grpc.BalanceResponse.BdmBalances
import com.bdmplatform.api.grpc.{AccountRequest, AccountsApiGrpc, BalancesRequest, BlocksApiGrpc, DataRequest, ScriptData, TransactionsApiGrpc, TransactionsRequest}
...
import com.bdmplatform.api.http.{AddressApiRoute, ApiError}
import com.bdmplatform.common.state.ByteStr
import com.bdmplatform.common.utils.{Base58, EitherExt2}
import com.bdmplatform.features.api.{ActivationStatus, FeatureActivationStatus}
import com.bdmplatform.http.DebugMessage
import com.bdmplatform.it.Node
import com.bdmplatform.it.api.SyncHttpApi.sync
...
object SyncHttpApi extends Assertions {
  ...
  implicit class NodeExtSync(n: Node) extends Assertions with Matchers {
    import com.bdmplatform.it.api.AsyncHttpApi.{NodeAsyncHttpApi => async}
  ...
  implicit class NodesExtSync(nodes: Seq[Node]) {
    ...
    import com.bdmplatform.it.api.AsyncHttpApi.{NodesAsyncHttpApi => async}
    ...
    def rollback(height: Int, returnToUTX: Boolean = true): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.bdmplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollback(height, returnToUTX)
        },
        ConditionAwaitTime
      )
    }
    ...
    def waitForHeight(height: Int): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.bdmplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).waitForHeight(height)
        },
        ConditionAwaitTime
      )
    }
  ...
  class NodeExtGrpc(n: Node) {
    import com.bdmplatform.account.{Address => Addr}
    import com.bdmplatform.it.api.AsyncHttpApi.{NodeAsyncHttpApi => async}
    ...
    def bdmBalance(address: ByteString): BdmBalances = {
      accounts.getBalances(BalancesRequest.of(address, Seq(ByteString.EMPTY))).next().getBdm
    }
