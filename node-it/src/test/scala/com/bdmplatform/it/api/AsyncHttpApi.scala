package com.bdmplatform.it.api
...
import com.bdmplatform.account.{AddressScheme, Alias, KeyPair}
import com.bdmplatform.api.grpc.BalanceResponse.BdmBalances
import com.bdmplatform.api.grpc.{AccountsApiGrpc, BalanceResponse, BalancesRequest, BlockRequest, BlocksApiGrpc, DataEntryResponse, DataRequest, TransactionResponse, TransactionsApiGrpc, TransactionsRequest}
import com.bdmplatform.api.http.RewardApiRoute.RewardStatus
import com.bdmplatform.api.http.assets._
import com.bdmplatform.api.http.{AddressApiRoute, ConnectReq}
import com.bdmplatform.common.utils.{Base58, Base64, EitherExt2}
import com.bdmplatform.crypto
import com.bdmplatform.features.api.ActivationStatus
import com.bdmplatform.http.DebugMessage._
import com.bdmplatform.http.{DebugMessage, RollbackParams, `X-Api-Key`}
import com.bdmplatform.it.Node
import com.bdmplatform.it.util.GlobalTimer.{instance => timer}
import com.bdmplatform.it.util._
import com.bdmplatform.lang.script.{ScriptReader, Script}
...
import com.bdmplatform.protobuf.block.PBBlocks
import com.bdmplatform.protobuf.transaction._
import com.bdmplatform.state.{AssetDistribution, AssetDistributionPage, DataEntry, Portfolio}
import com.bdmplatform.transaction.assets.exchange.Order
import com.bdmplatform.transaction.assets.{BurnTransaction, IssueTransaction, SetAssetScriptTransaction, SponsorFeeTransaction}
import com.bdmplatform.transaction.lease.{LeaseCancelTransaction, LeaseTransaction}
...
import com.bdmplatform.transaction.{CreateAliasTransaction, DataTransaction}
...
object AsyncHttpApi extends Assertions {
  ...
  implicit class NodeAsyncHttpApi(val n: Node) extends Assertions with Matchers {
    ...
    def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long): Future[Transaction] =
      postJson("/bdm/payment", PaymentRequest(amount, fee, sourceAddress, recipient)).as[Transaction]
    ...
    def debugStateAt(height: Long): Future[Map[String, Long]] = getWithApiKey(s"/debug/stateBdm/$height").as[Map[String, Long]]
  ...
  class NodeExtGrpc(n: Node) {
    ...
    import com.bdmplatform.protobuf.transaction.{Transaction => PBTransaction, Script => PBScript}
    ...
    def broadcastTransfer(source: KeyPair,
                          ...
                          version: Int = 2,
                          assetId: String = "BDM",
                          feeAssetId: String = "BDM",
                          attachment: ByteString = ByteString.EMPTY,
                          timestamp: Long = System.currentTimeMillis): Future[PBSignedTransaction] = {
      val unsigned = PBTransaction(
        chainId,
        ByteString.copyFrom(source.publicKey),
        Some(Amount.of(if (feeAssetId == "BDM") ByteString.EMPTY else ByteString.copyFrom(Base58.decode(feeAssetId)), fee)),
        timestamp,
        version,
        PBTransaction.Data.Transfer(TransferTransactionData.of(
          Some(recipient),
          Some(Amount.of(if (assetId == "BDM") ByteString.EMPTY else ByteString.copyFrom(Base58.decode(assetId)), amount)),
          attachment
        ))
      )
    ...
    def exchange(matcher: KeyPair,
                 ...
                 timestamp: Long,
                 version: Byte,
                 matcherFeeAssetId: String = "BDM"): Future[PBSignedTransaction] = {

      val unsigned = PBTransaction(
        chainId,
        ByteString.copyFrom(matcher.publicKey),
        Some(Amount.of(if (matcherFeeAssetId == "BDM") ByteString.EMPTY else ByteString.copyFrom(Base58.decode(matcherFeeAssetId)), fee)),
        timestamp,
        version,
    ...
    def bdmBalance(address: ByteString): Future[BdmBalances] = {
      def createCallObserver[T]: (StreamObserver[T], Task[List[T]]) = {
        val subj = ConcurrentSubject.publishToOne[T]
      ...
      result.map(_.headOption.getOrElse(throw new NoSuchElementException("Balances not found for address")).getBdm).runToFuture
    }
