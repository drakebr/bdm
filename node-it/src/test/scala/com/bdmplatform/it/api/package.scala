...
package object api {
  ...
  implicit val addressReads: Reads[com.bdmplatform.account.Address] = Reads {
  ...
  implicit val dstMapReads: Reads[Map[com.bdmplatform.account.Address, Long]] = Reads { json =>
    json.validate[Map[String, Long]].map { dst =>
      dst.map {
        case (addrStr, balance) =>
          com.bdmplatform.account.Address.fromString(addrStr).explicitGet() -> balance
      }
    }
  }
  ...
  implicit val distributionReads: Reads[AssetDistribution] = Reads { json =>
    json
      .validate[Map[com.bdmplatform.account.Address, Long]]
  ...
  implicit val distributionPageReads: Reads[AssetDistributionPage] = Reads { json =>
    json.validate[Paged[com.bdmplatform.account.Address, AssetDistribution]].map(pg => AssetDistributionPage(pg))
  }
}
