...
trait GrpcIntegrationSuiteWithThreeAddress
  ...
  abstract protected override def beforeAll(): Unit = {
    ...
    val defaultBalance: Long = 100.bdm

    def dumpBalances(node: Node, accounts: Seq[ByteString], label: String): Unit = {
      accounts.foreach(acc => {
        val balance = miner.grpc.bdmBalance(acc).available
        val eff = miner.grpc.bdmBalance(acc).effective
    ...
    def correctStartBalancesFuture(): Unit = {
      ...
      accounts.foreach(acc => miner.grpc.bdmBalance(acc).available shouldBe defaultBalance)
      accounts.foreach(acc => miner.grpc.bdmBalance(acc).effective shouldBe defaultBalance)
    }
