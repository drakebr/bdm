...
//  credentials += Credentials(Path.userHome / ".sbt" / ".credentials"),
  ...
  developers := List(
    Developer("petermz", "Peter Zhelezniakov", "peterz@rambler.ru", url("https://bdmplatform.com"))
  )
)
...
lazy val node             = project.dependsOn(langJVM, `lang-testkit` % "test", langJS, langTests)
...
inScope(Global)(
  Seq(
    scalaVersion := "2.12.9",
    organization := "com.bdmplatform",
    organizationName := "Bdm Platform",
    V.fallback := (1, 1, 7),
    organizationHomepage := Some(url("https://bdmplatform.com")),
    scmInfo := Some(ScmInfo(url("https://github.com/bdmplatform/Bdm"), "git@github.com:bdmplatform/Bdm.git", None)),
    licenses := Seq(("MIT", url("https://github.com/bdmplatform/Bdm/blob/master/LICENSE"))),
    scalacOptions ++= Seq(
      "-feature",
      "-deprecation",
      "-unchecked",
