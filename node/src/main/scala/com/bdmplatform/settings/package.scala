...
package object settings {
  ...
  def loadConfig(maybeUserConfig: Option[Config]): Config = {
    ...
    val cmdDefaults =
      Try(withApp.getConfig("bdm.defaults"))
        .getOrElse(ConfigFactory.empty())
        .atPath("bdm")

    external
      .withFallback(cmdDefaults)
      .withFallback(ConfigFactory.parseString(s"bdm.directory = ${defaultDirectory(withApp)}"))
      .withFallback(ConfigFactory.defaultApplication())
      .withFallback(ConfigFactory.defaultReference())
      .resolve()
  }
  ...
  def defaultDirectory(config: Config): String = {
    ...
    def withNetwork(config: Config): String = {
      val bc = config.getString("bdm.blockchain.type")
      val suffix =
        if (bc == "CUSTOM") {
          val char = config.getString("bdm.blockchain.custom.address-scheme-character").headOption.getOrElse(0.toChar)
