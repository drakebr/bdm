...
import com.bdmplatform.account.{KeyPair, PrivateKey, PublicKey}
import com.bdmplatform.common.utils.{Base58, Base64, FastBase58}
...
object UtilApp {
  ...
  private[this] lazy val commandParser = {
    ...
    OParser.sequence(
      programName("bdm util"),
      head("Bdm Util", Version.VersionString),
      OParser.sequence(
  ...
  private[this] object Actions {
    ...
    def doCompile(settings: BdmSettings)(c: Command, str: Array[Byte]): ActionResult = {
    ...
    def doSign(c: Command, data: Array[Byte]): ActionResult =
      Right(com.bdmplatform.crypto.sign(c.signOptions.privateKey, data))

    def doVerify(c: Command, data: Array[Byte]): ActionResult =
      Either.cond(com.bdmplatform.crypto.verify(c.verifyOptions.signature, data, c.verifyOptions.publicKey), data, "Invalid signature")
    ...
    def doHash(c: Command, data: Array[Byte]): ActionResult = c.hashOptions.mode match {
      case "fast"   => Right(com.bdmplatform.crypto.fastHash(data))
      case "secure" => Right(com.bdmplatform.crypto.secureHash(data))
