...
import com.bdmplatform.account.{Address, AddressOrAlias, PublicKey}
import com.bdmplatform.lang.ValidationError
import com.bdmplatform.transaction.Asset.Bdm
import com.bdmplatform.transaction.{Asset, ProvenTransaction, TxValidationError, VersionedTransaction}
...
object LeaseTransaction {
  ...
  def validateLeaseParams(amount: Long, fee: Long, recipient: AddressOrAlias, sender: PublicKey): Either[ValidationError, Unit] =
    if (amount <= 0) {
      Left(TxValidationError.NonPositiveAmount(amount, "bdm"))
