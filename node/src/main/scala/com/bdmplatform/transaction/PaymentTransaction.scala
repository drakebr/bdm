...
object PaymentTransaction extends TransactionParserFor[PaymentTransaction] with TransactionParser.HardcodedVersion1 {
  ...
  def create(sender: PublicKey,
    ...
    if (amount <= 0) {
      Left(TxValidationError.NonPositiveAmount(amount, "bdm")) //CHECK IF AMOUNT IS POSITIVE
    } else if (fee <= 0) {
  ...
  override protected def parseTail(bytes: Array[Byte]): Try[TransactionT] = {
    Try {
      ...
      byteTailDescription.deserializeFromByteArray(bytes).flatMap { tx =>
        (
          if (tx.amount <= 0) {
            Left(TxValidationError.NonPositiveAmount(tx.amount, "bdm")) //CHECK IF AMOUNT IS POSITIVE
          } else if (tx.fee <= 0) {
            Left(TxValidationError.InsufficientFee) //CHECK IF FEE IS POSITIVE
