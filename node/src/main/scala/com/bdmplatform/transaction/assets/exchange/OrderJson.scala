...
import com.bdmplatform.transaction.{Asset, Proofs}
...
object OrderJson {
  ...
  private val assetReads: Reads[Asset] = {
    case JsNull | JsString("") => JsSuccess(Bdm)
    case JsString(s) =>
  ...
  implicit val assetPairReads: Reads[AssetPair] = {
    val r = (JsPath \ "amountAsset").readWithDefault[Asset](Bdm)(assetReads) and
      (JsPath \ "priceAsset").readWithDefault[Asset](Bdm)(assetReads)
    r(AssetPair(_, _))
  }
