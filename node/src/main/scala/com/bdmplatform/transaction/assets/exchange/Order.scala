package com.bdmplatform.transaction.assets.exchange
...
import com.bdmplatform.serialization.{BytesSerializable, JsonSerializable}
...
import com.bdmplatform.transaction.assets.exchange.Validation.booleanOperators
import com.bdmplatform.utils.byteStrWrites
...
trait Order extends BytesSerializable with JsonSerializable with Proven {
  ...
  def matcherFeeAssetId: Asset = Bdm
  ...
  def getSpendAmount(matchAmount: Long, matchPrice: Long): Either[ValidationError, Long] =
    Try {
      ...
      if (orderType == OrderType.SELL) matchAmount
        ...
        if (getSpendAssetId == Bdm && !(spend + matcherFee).isValidLong) {
  ...
  override def toString: String = {
    val matcherFeeAssetIdStr = if (version == 3) s" matcherFeeAssetId=${matcherFeeAssetId.fold("Bdm")(_.toString)}," else ""
...
object Order {
  ...
  def apply(senderPublicKey: PublicKey,
            ...
            proofs: Proofs,
            version: Byte = 1,
            matcherFeeAssetId: Asset = Asset.Bdm): Order = version match {
  ...
  def buy(sender: KeyPair,
          ...
          matcherFee: Long,
          version: Byte = 1,
          matcherFeeAssetId: Asset = Bdm): Order = {
