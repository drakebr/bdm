...
import com.bdmplatform.lang.v1.traits.domain.Recipient._
...
object BdmEnvironment {
  type In = Transaction :+: Order :+: ScriptTransfer :+: CNil
}

class BdmEnvironment(nByte: Byte, in: Coeval[BdmEnvironment.In], h: Coeval[Int], blockchain: Blockchain, address: Coeval[ByteStr])
  ...
  override def data(recipient: Recipient, key: String, dataType: DataType): Option[Any] = {
    for {
      address <- recipient match {
        case Address(bytes) =>
          com.bdmplatform.account.Address
            .fromBytes(bytes.arr)
            .toOption
        case Alias(name) =>
          com.bdmplatform.account.Alias
            .create(name)
  ...
  override def resolveAlias(name: String): Either[String, Recipient.Address] =
    blockchain
      .resolveAlias(com.bdmplatform.account.Alias.create(name).explicitGet())
      .left
      .map(_.toString)
      .right
  ...
  override def accountBalanceOf(addressOrAlias: Recipient, maybeAssetId: Option[Array[Byte]]): Either[String, Long] = {
    (for {
      aoa <- addressOrAlias match {
        ...
        case Alias(name)    => com.bdmplatform.account.Alias.create(name)
      }
