...
import com.bdmplatform.state.patch.{CancelAllLeases, CancelInvalidLeaseIn, CancelLeaseOverflow}
import com.bdmplatform.state.reader.CompositeBlockchain
import com.bdmplatform.transaction.TxValidationError.{ActivationError, _}
...
object BlockDiffer extends ScorexLogging {
  ...
  def fromBlockTraced[Constraint <: MiningConstraint](blockchain: Blockchain,
    ...
    val minerRewardDistr: Portfolio =
        blockchain.lastBlockReward.map(Portfolio.build(Asset.Bdm, _)).getOrElse(Portfolio.empty)
            ...
            balance = portfolio.balance +
              portfolio.assets.map {
                case (assetId, assetFee) =>
                  ...
                  Sponsorship.toBdm(assetFee, baseFee)
              }.sum)
        else portfolio
        ...
        case (TracedResult(Right(Result(currDiff, carryFee, currTotalFee, currConstraint)), _), tx) =>
          ...
          txDiffer(currBlockchain, tx).flatMap { newDiff =>
            ...
            if (updatedConstraint.isOverfilled)
              ...
              val totalBdmFee                = currTotalFee + curBlockFees.balance + nextBlockFee
              ...
              Right(Result(currDiff.combine(resultDiff), resultCarryFee, totalBdmFee, updatedConstraint))
            }
          }
      }
      .map { result =>
