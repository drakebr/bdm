...
import com.bdmplatform.lang.v1.ContractLimits
...
import com.bdmplatform.lang.v1.evaluator.{ContractEvaluator, LogItem, ScriptResult}
import com.bdmplatform.lang.v1.traits.Environment
import com.bdmplatform.lang.v1.traits.domain.Tx.ScriptTransfer
import com.bdmplatform.lang.v1.traits.domain.{DataItem, Recipient}
import com.bdmplatform.metrics._
import com.bdmplatform.settings.Constants
import com.bdmplatform.state._
import com.bdmplatform.state.diffs.CommonValidation._
import com.bdmplatform.state.diffs.FeeValidation._
...
import com.bdmplatform.transaction.smart.script.ScriptRunner
import com.bdmplatform.transaction.smart.script.ScriptRunner.TxOrd
import com.bdmplatform.transaction.smart.script.trace.{AssetVerifierTrace, InvokeScriptTrace, TracedResult}
import com.bdmplatform.transaction.smart.{InvokeScriptTransaction, BdmEnvironment}
      ...
      case Right(Some(sc @ ContractScriptImpl(_, contract))) =>
        val scriptResultE =
          stats.invokedScriptExecution.measureForType(InvokeScriptTransaction.typeId)({
            val environment = new BdmEnvironment(
        ...
        for {
          ...
          feeInfo <- TracedResult(tx.assetFee._1 match {
            case Bdm => Right((tx.fee, Map(tx.sender.toAddress -> Portfolio(-tx.fee, LeaseBalance.empty, Map.empty))))
            case asset @ IssuedAsset(_) =>
              for {
                ...
                bdmFee <- Either.cond(
                  ...
                  Sponsorship.toBdm(tx.fee, assetInfo.sponsorship),
                ...
                (bdmFee,
                 Map(
                   ...
                   assetInfo.issuer.toAddress -> Portfolio(-bdmFee, LeaseBalance.empty, Map(asset -> tx.fee))
                 ))
              }
          })
          ...
          _ <- TracedResult {
            ...
            val minBdm  = totalScriptsInvoked * ScriptExtraFee + FeeConstants(InvokeScriptTransaction.typeId) * FeeUnit
            val txName    = Constants.TransactionNames(InvokeScriptTransaction.typeId)
            val assetName = tx.assetFee._1.fold("BDM")(_.id.toString)
            Either.cond(
              minBdm <= bdmFee,
              (),
              GenericError(s"Fee in $assetName for $txName (${tx.assetFee._2} in $assetName)" +
                s" with $totalScriptsInvoked total scripts invoked does not exceed minimal value of $minBdm BDM.")
            )
          }
