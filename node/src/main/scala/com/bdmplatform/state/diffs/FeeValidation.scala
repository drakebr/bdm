...
import com.bdmplatform.transaction.smart._
import com.bdmplatform.transaction.transfer._

object FeeValidation {

  case class FeeDetails(asset: Asset, requirements: Chain[String], minFeeInAsset: Long, minFeeInBdm: Long)
  ...
  private def notEnoughFeeError(txType: Byte, feeDetails: FeeDetails, feeAmount: Long): ValidationError = {
    ...
    val actualFee   = s"$feeAmount in ${feeDetails.asset.fold("BDM")(_.id.base58)}"
    val requiredFee = s"${feeDetails.minFeeInBdm} BDM${feeDetails.asset.fold("")(id => s" or ${feeDetails.minFeeInAsset} ${id.id.base58}")}"
  ...
  private case class FeeInfo(assetInfo: Option[(IssuedAsset, AssetDescription)], requirements: Chain[String], bdmFee: Long)
  ...
  private def feeAfterSponsorship(txAsset: Asset, height: Int, blockchain: Blockchain, tx: Transaction): Either[ValidationError, FeeInfo] = {
    if (height < Sponsorship.sponsoredFeesSwitchHeight(blockchain)) {
      ...
      for {
        ...
        r <- txAsset match {
          case Bdm => Right(FeeInfo(None, Chain.empty, feeInUnits * FeeUnit))
          case assetId @ IssuedAsset(_) =>
            ...
            } yield FeeInfo(Some((assetId, assetInfo)), Chain.empty, bdmFee)
        }
      } yield r
    }
  }
  ...
  def getMinFee(blockchain: Blockchain, height: Int, tx: Transaction): Either[ValidationError, FeeDetails] = {
    feeAfterSponsorship(tx.assetFee._1, height, blockchain, tx)
      ...
      .map {
        case FeeInfo(Some((assetId, assetInfo)), reqs, amountInBdm) =>
          FeeDetails(assetId, reqs, Sponsorship.fromBdm(amountInBdm, assetInfo.sponsorship), amountInBdm)
        case FeeInfo(None, reqs, amountInBdm) =>
          FeeDetails(Bdm, reqs, amountInBdm, amountInBdm)
      }
  }
}
