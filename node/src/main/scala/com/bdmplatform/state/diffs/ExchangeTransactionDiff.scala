...
import com.bdmplatform.transaction.TxValidationError.{GenericError, OrderValidationError}
import com.bdmplatform.transaction.assets.exchange.{ExchangeTransaction, Order, OrderV3}
...
object ExchangeTransactionDiff {
  ...
  def bdmPortfolio(amt: Long) = Portfolio(amt, LeaseBalance.empty, Map.empty)

  def getAssetPortfolio(asset: Asset, amt: Long): Portfolio = {
    asset.fold(bdmPortfolio(amt))(assetId => Portfolio(0, LeaseBalance.empty, Map(assetId -> amt)))
  }

  /*** Calculates fee portfolio from the order (taking into account that in OrderV3 fee can be paid in asset != Bdm) */
