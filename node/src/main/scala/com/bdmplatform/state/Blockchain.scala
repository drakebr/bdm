...
import com.bdmplatform.state.extensions.{AddressTransactions, BlockchainExtensions, Distributions}
...
trait Blockchain {
  ...
  def bdmAmount(height: Int): BigInt
  ...
  def balanceOnlySnapshots(address: Address, height: Int, assetId: Asset = Bdm): Option[(Int, Long)]
  ...
  def balance(address: Address, mayBeAssetId: Asset = Bdm): Long
