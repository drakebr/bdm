...
import com.bdmplatform.account.{Address, Alias, PublicKey}
...
object Sponsorship {
  ...
  def calcBdmFeeAmount(tx: Transaction, getSponsorship: IssuedAsset => Option[Long]): Long = tx.assetFee match {
    case (asset @ IssuedAsset(_), amountInAsset) =>
      val sponsorship = getSponsorship(asset).getOrElse(0L)
      Sponsorship.toBdm(amountInAsset, sponsorship)

    case (Asset.Bdm, amountInBdm) =>
      amountInBdm
  }
  ...
  def fromBdm(bdmFee: Long, sponsorship: Long): Long = {
    if (bdmFee == 0 || sponsorship == 0) return 0
    val assetFee = BigInt(bdmFee) * sponsorship / FeeValidation.FeeUnit
