...
final case class CompositeBlockchain(
  ...
  override def balanceOnlySnapshots(address: Address, h: Int, assetId: Asset = Bdm): Option[(Int, Long)] = {
  ...
  override def bdmAmount(height: Int): BigInt = inner.bdmAmount(height)
}
