...
import com.bdmplatform.block.{Block, BlockHeader, MicroBlock}
import com.bdmplatform.common.state.ByteStr
import com.bdmplatform.crypto.{verify => sigVerify}
...
import com.bdmplatform.metrics.{TxsInBlockchainStats, _}
import com.bdmplatform.mining.{MiningConstraint, MiningConstraints, MultiDimensionalMiningConstraint}
import com.bdmplatform.settings.{BlockchainSettings, BdmSettings}
import com.bdmplatform.state.diffs.BlockDiffer
import com.bdmplatform.state.extensions.composite.{CompositeAddressTransactions, CompositeDistributions}
import com.bdmplatform.state.extensions.{AddressTransactions, Distributions}
import com.bdmplatform.state.reader.{CompositeBlockchain, LeaseDetails}
import com.bdmplatform.transaction.Asset.{IssuedAsset, Bdm}
import com.bdmplatform.transaction.TxValidationError.{BlockAppendError, GenericError, MicroBlockAppendError}
...
class BlockchainUpdaterImpl(
  ...
  import bdmSettings.blockchainSettings.functionalitySettings
  ...
  private lazy val maxBlockReadinessAge = bdmSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis
  ...
  override val settings: BlockchainSettings = bdmSettings.blockchainSettings
  ...
  private def featuresApprovedWithBlock(block: Block): Set[Short] = {
    ...
    if (height % featuresCheckPeriod == 0) {
      ...
      if (unimplementedActivated.nonEmpty) {
        ...
        if (bdmSettings.featuresSettings.autoShutdownOnUnsupportedFeature) {
  ...
  override def processBlock(block: Block, verify: Boolean = true): Either[ValidationError, Option[DiscardedTransactions]] = writeLock {
    ...
    Either
      .cond(
        !bdmSettings.featuresSettings.autoShutdownOnUnsupportedFeature || notImplementedFeatures.isEmpty,
        (),
            ...
            _ map {
              case (BlockDiffer.Result(newBlockDiff, carry, totalFee, updatedTotalConstraint), discarded, reward) =>
                ...
                if ((block.timestamp > time
                      .getTimestamp() - bdmSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis) || (newHeight % 100 == 0)) {
  ...
  override def bdmAmount(height: Int): BigInt = readLock {
    ngState match {
      case Some(ng) if this.height == height =>
        blockchain.bdmAmount(height - 1) + ng.reward.map(BigInt(_)).getOrElse(BigInt(0))
      case _ => blockchain.bdmAmount(height)
    }
  }
  ...
  override def balanceOnlySnapshots(address: Address, h: Int, assetId: Asset = Bdm): Option[(Int, Long)] = readLock {
  ...
  override def collectLposPortfolios[A](pf: PartialFunction[(Address, Portfolio), A]): Map[Address, A] = readLock {
    ngState.fold(blockchain.collectLposPortfolios(pf)) { ng =>
      ...
      for ((a, p) <- ng.bestLiquidDiff.portfolios if p.lease != LeaseBalance.empty || p.balance != 0) {
        pf.runWith(b += a -> _)(a -> this.bdmPortfolio(a))
      }
