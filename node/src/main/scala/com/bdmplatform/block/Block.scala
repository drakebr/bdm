...
import com.bdmplatform.block.fields.{FeaturesBlockField, RewardBlockField}
import com.bdmplatform.common.state.ByteStr
import com.bdmplatform.common.utils.EitherExt2
import com.bdmplatform.consensus.nxt.{NxtConsensusBlockField, NxtLikeConsensusBlockData}
import com.bdmplatform.crypto
import com.bdmplatform.crypto._
import com.bdmplatform.lang.ValidationError
import com.bdmplatform.settings.GenesisSettings
...
case class Block private[block] (override val timestamp: Long,
  ...
  val json: Coeval[JsObject] = Coeval.evalOnce(
    BlockHeader.json(this, bytes().length) ++
      Json.obj("fee" -> transactionData.map(_.assetFee).collect { case (Bdm, feeAmt) => feeAmt }.sum) ++
  ...
  val feesPortfolio: Coeval[Portfolio] = Coeval.evalOnce(Monoid[Portfolio].combineAll({
    ...
    case (assetId, feeVolume) =>
      assetId match {
        case Bdm                  => Portfolio(feeVolume, LeaseBalance.empty, Map.empty)
