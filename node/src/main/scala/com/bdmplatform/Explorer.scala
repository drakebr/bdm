...
import com.bdmplatform.database.{DBExt, Keys, LevelDBWriter, openDB}
import com.bdmplatform.settings.Constants
import com.bdmplatform.state.{Height, TxNum}
import com.bdmplatform.transaction.Asset.IssuedAsset
import com.bdmplatform.transaction.{Transaction, TransactionParsers}
...
object Explorer extends ScorexLogging {
  ...
  def main(argsRaw: Array[String]): Unit = {
    if (argsRaw.isEmpty) {
      System.err.println("Usage: bdm explore <command> [args] [--config|-c <cfg file>]")
      return
    }
    ...
    try {
      ...
      flag match {
        case "WB" =>
          ...
          val expectedTotalBalance = Constants.UnitsInWave * Constants.TotalBdm + actualTotalReward
          val byKeyTotalBalance = reader.bdmAmount(blockchainHeight)

          if (actualTotalBalance != expectedTotalBalance || expectedTotalBalance != byKeyTotalBalance)
            log.error(s"Something wrong, actual total bdm balance: $actualTotalBalance," +
              s" expected total bdm balance: $expectedTotalBalance, total bdm balance by key: $byKeyTotalBalance")
          else
            log.info(s"Correct total bdm balance: $actualTotalBalance WAVELETS")
        ...
        case "A" =>
          ...
          val balances = wbh.map { h =>
            val k = Keys.bdmBalance(addressId)(h)
        ...
        case "AP" =>
          ...
          log.info(s"$address : ${portfolio.balance} BDM, ${portfolio.lease}, ${portfolio.assets.size} assets")
        ...
        case "APS" =>
          ...
          println(s"\nBDM balances\n")
          for {
            seqNr     <- (1 to db.get(Keys.addressesForBdmSeqNr)).par
            addressId <- db.get(Keys.addressesForBdm(seqNr)).par
            history = db.get(Keys.bdmBalanceHistory(addressId))
            actualHeight <- history.headOption
            balance = db.get(Keys.bdmBalance(addressId)(actualHeight))
            if balance > 0
          } yield {
