...
import com.bdmplatform.account.{Address, AddressScheme}
import _root_.com.bdmplatform.account.{Address, AddressScheme}
import com.bdmplatform.actor.RootActorSystem
...
import com.bdmplatform.api.http.leasing.LeaseApiRoute
...
import com.bdmplatform.consensus.nxt.api.http.NxtConsensusApiRoute
import com.bdmplatform.database.{DBExt, Keys, openDB}
import com.bdmplatform.extensions.{Context, Extension}
import com.bdmplatform.features.EstimatorProvider._
import com.bdmplatform.features.api.ActivationApiRoute
import com.bdmplatform.history.StorageFactory
import com.bdmplatform.http.{DebugApiRoute, NodeApiRoute}
import com.bdmplatform.lang.ValidationError
import com.bdmplatform.metrics.Metrics
import com.bdmplatform.mining.{Miner, MinerImpl}
import com.bdmplatform.network.RxExtensionLoader.RxExtensionLoaderShutdownHook
import com.bdmplatform.network._
import com.bdmplatform.settings.BdmSettings
import com.bdmplatform.state.Blockchain
import com.bdmplatform.state.appender.{BlockAppender, ExtensionAppender, MicroblockAppender}
import com.bdmplatform.transaction.smart.script.trace.TracedResult
import com.bdmplatform.transaction.{Asset, Transaction}
import com.bdmplatform.utils.Schedulers._
import com.bdmplatform.utils._
import com.bdmplatform.utx.{UtxPool, UtxPoolImpl}
...
class Application(val actorSystem: ActorSystem, val settings: BdmSettings, configRoot: ConfigObject, time: NTP) extends ScorexLogging {
  app =>
  ...
  def run(): Unit = {
    ...
    val extensionContext = new Context {
      override def settings: BdmSettings = app.settings
    ...
    if (settings.restAPISettings.enable) {
      if (settings.restAPISettings.apiKeyHash == "H6nsiifwYKYEx6YzYD7woP1XCn72RVvx6tC1zjjLXqsu") {
        log.error(
          "Usage of the default api key hash (H6nsiifwYKYEx6YzYD7woP1XCn72RVvx6tC1zjjLXqsu) is prohibited, please change it in the bdm.conf"
        )
      ...
      def loadBalanceHistory(address: Address): Seq[(Int, Long)] = db.readOnly { rdb =>
        rdb.get(Keys.addressId(address)).fold(Seq.empty[(Int, Long)]) { aid =>
          rdb.get(Keys.bdmBalanceHistory(aid)).map { h =>
            h -> rdb.get(Keys.bdmBalance(aid)(h))
          }
        }
      }
...
object Application {
  private[bdmplatform] def loadApplicationConfig(external: Option[File] = None): BdmSettings = {
    import com.bdmplatform.settings._

    val config = loadConfig(external.map(ConfigFactory.parseFile))

    // DO NOT LOG BEFORE THIS LINE, THIS PROPERTY IS USED IN logback.xml
    System.setProperty("bdm.directory", config.getString("bdm.directory"))
    if (config.hasPath("bdm.config.directory")) System.setProperty("bdm.config.directory", config.getString("bdm.config.directory"))
  ...
  def main(args: Array[String]): Unit = {
    ...
    args.headOption.getOrElse("") match {
      ...
      case "help" | "--help" | "-h" => println("Usage: bdm <config> | export | import | explore | util")
      case _                        => startNode(args.headOption)
    }
  }

  private[this] def startNode(configFile: Option[String]): Unit = {
    import com.bdmplatform.settings.Constants
    ...
    RootActorSystem.start("bdmplatform", settings.config) { actorSystem =>
