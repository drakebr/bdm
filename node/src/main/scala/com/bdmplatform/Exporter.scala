...
object Exporter extends ScorexLogging {
  private[bdmplatform] object Formats {
    val Binary   = "BINARY"
  ...
  private[this] final case class ExporterOptions(configFileName: File = new File("bdm-testnet.conf"),
  ...
  private[this] lazy val commandParser = {
    ...
    OParser.sequence(
      programName("bdm export"),
      head("Bdm Blockchain Exporter", Version.VersionString),
      opt[File]('c', "config")
