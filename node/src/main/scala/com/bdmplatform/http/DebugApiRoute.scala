...
import com.bdmplatform.api.http.ApiError.InvalidAddress
...
import com.bdmplatform.mining.{Miner, MinerDebugInfo}
import com.bdmplatform.network.{PeerDatabase, PeerInfo, _}
import com.bdmplatform.settings.BdmSettings
import com.bdmplatform.state.diffs.TransactionDiffer
import com.bdmplatform.state.extensions.Distributions
import com.bdmplatform.state.{Blockchain, LeaseBalance, NG, TransactionId}
import com.bdmplatform.transaction.TxValidationError.InvalidRequestSignature
import com.bdmplatform.transaction._
import com.bdmplatform.transaction.smart.script.trace.TracedResult
import com.bdmplatform.transaction.smart.{InvokeScriptTransaction, Verifier}
...
case class DebugApiRoute(
  ...
  private lazy val bdmConfig: JsObject = Json.obj("bdm" -> (fullConfig \ "bdm").get)

  override val settings = ws.restAPISettings
  override lazy val route: Route = pathPrefix("debug") {
    stateChanges ~ balanceHistory ~ withAuth {
      blocks ~ state ~ info ~ stateBdm ~ rollback ~ rollbackTo ~ blacklist ~ portfolios ~ minerInfo ~ historyInfo ~ configInfo ~ print ~ validate
    }
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.bdmplatform.http.DebugMessage",
        defaultValue = "{\n\t\"message\": \"foo\"\n}"
      )
    )
  )
  ...
  @ApiOperation(
    value = "Bdm balance history",
    notes = "Bdm balance history",
    httpMethod = "GET"
  )
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        name = "address",
        value = "An address to load bdm balance history for",
        required = true,
        dataType = "string",
        paramType = "path"
      )
  ...
  private def bdmDistribution(height: Int): Route =
    optionalHeaderValueByType[Accept](()) {
      case Some(accept) if accept.mediaRanges.exists(CustomJson.acceptsNumbersAsStrings) =>
        complete(dst.bdmDistribution(height).map(_.map { case (a, b) => a.stringRepr -> b.toString }))
      case _ =>
        complete(dst.bdmDistribution(height).map(_.map { case (a, b) => a.stringRepr -> b }))
    }
  ...
  def state: Route = (path("state") & get) {
    bdmDistribution(ng.height)
  }

  @Path("/stateBdm/{height}")
  ...
  def stateBdm: Route = (path("stateBdm" / IntNumber) & get) { height =>
    bdmDistribution(height)
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.bdmplatform.http.RollbackParams",
  ...
  def configInfo: Route = (path("configInfo") & get & parameter('full.as[Boolean])) { full =>
    complete(if (full) fullConfig else bdmConfig)
  }
