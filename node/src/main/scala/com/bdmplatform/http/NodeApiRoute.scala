...
import com.bdmplatform.Shutdownable
import com.bdmplatform.api.http.{ApiRoute, AuthRoute}
import com.bdmplatform.settings.{Constants, RestAPISettings}
...
case class NodeApiRoute(settings: RestAPISettings, blockchain: Blockchain, application: Shutdownable)
  ...
  @Path("/version")
  @ApiOperation(value = "Version", notes = "Get Bdm node version", httpMethod = "GET")
  @ApiResponses(
    Array(
      new ApiResponse(code = 200, message = "Json Bdm node version")
    )
  )
