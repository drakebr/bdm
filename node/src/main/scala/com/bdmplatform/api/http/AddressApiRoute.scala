...
import com.bdmplatform.common.utils.{Base58, Base64}
import com.bdmplatform.crypto
import com.bdmplatform.http.BroadcastRoute
import com.bdmplatform.lang.contract.meta.Dic
import com.bdmplatform.lang.{Global, ValidationError}
import com.bdmplatform.network.UtxPoolSynchronizer
import com.bdmplatform.protobuf.api
...
import com.bdmplatform.transaction.{Asset, TransactionFactory}
...
case class AddressApiRoute(settings: RestAPISettings, wallet: Wallet, blockchain: Blockchain, utxPoolSynchronizer: UtxPoolSynchronizer, time: Time)
  ...
  @ApiImplicitParams(
    Array(
      ...
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.bdmplatform.api.http.SignedMessage",
        defaultValue =
  ...
  def balances: Route = (path("balance") & get & parameters('height.as[Int].?) & parameters('address.*) & parameters('asset.?)) {
    (height, addresses, assetId) =>
      complete(balancesJson(height.getOrElse(blockchain.height), addresses.toSeq, assetId.fold(Bdm: Asset)(a => IssuedAsset(Base58.decode(a)))))
  }
  ...
  def balancesPost: Route = (path("balance") & (post & entity(as[JsObject]))) { request =>
    ...
    complete(balancesJson(height.getOrElse(blockchain.height), addresses, assetId.fold(Bdm: Asset)(a => IssuedAsset(Base58.decode(a)))))
  }
