...
import com.bdmplatform.state.{Blockchain, BlockchainExt, DataEntry, Height}
...
import com.bdmplatform.transaction.lease.{LeaseTransaction, LeaseTransactionV1, LeaseTransactionV2}
...
class CommonAccountApi(blockchain: Blockchain) {
  ...
  def balanceDetails(address: Address): BalanceDetails = {
    val portfolio = blockchain.bdmPortfolio(address)
    BalanceDetails(
      portfolio.balance,
  ...
  def portfolio(address: Address): Map[Asset, Long] = {
    ...
    portfolio.assets ++ Map(Asset.Bdm -> portfolio.balance)
  }
