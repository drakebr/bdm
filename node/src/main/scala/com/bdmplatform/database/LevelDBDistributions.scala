...
import com.bdmplatform.state.{AddressId, AssetDistribution, AssetDistributionPage, Portfolio}
...
private[database] final class LevelDBDistributions(ldb: LevelDBWriter) extends Distributions {
  ...
  import com.bdmplatform.features.FeatureProvider.FeatureProviderExt
  import ldb._
  ...
  override def bdmDistribution(height: Int): Either[ValidationError, Map[Address, Long]] = readOnly { db =>
    val canGetAfterHeight = db.get(Keys.safeRollbackHeight)
    ...
    Either.cond(
      height > canGetAfterHeight,
      createMap(),
      GenericError(s"Cannot get bdm distribution at height less than ${canGetAfterHeight + 1}")
    )
  }
