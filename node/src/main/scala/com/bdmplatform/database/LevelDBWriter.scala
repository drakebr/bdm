package com.bdmplatform.database
...
import com.bdmplatform.database.patch.DisableHijackedAliases
...
import com.bdmplatform.settings.{BlockchainSettings, Constants, DBSettings}
import com.bdmplatform.state.extensions.{AddressTransactions, Distributions}
import com.bdmplatform.state.reader.LeaseDetails
import com.bdmplatform.state.{TxNum, _}
import com.bdmplatform.transaction.Asset.{IssuedAsset, Bdm}
import com.bdmplatform.transaction.TxValidationError.{AliasDoesNotExist, AliasIsDisabled, GenericError}
...
class LevelDBWriter(
  ...
  protected override def loadBalance(req: (Address, Asset)): Long = readOnly { db =>
    addressId(req._1).fold(0L) { addressId =>
      req._2 match {
        ...
        case Bdm                  => db.fromHistory(Keys.bdmBalanceHistory(addressId), Keys.bdmBalance(addressId)).getOrElse(0L)
      }
    }
  }
  ...
  private[database] def loadLposPortfolio(db: ReadOnlyDB, addressId: BigInt) = Portfolio(
    db.fromHistory(Keys.bdmBalanceHistory(addressId), Keys.bdmBalance(addressId)).getOrElse(0L),
  ...
  override def bdmAmount(height: Int): BigInt = readOnly { db =>
    if (db.has(Keys.bdmAmount(height))) db.get(Keys.bdmAmount(height))
    else BigInt(Constants.UnitsInWave * Constants.TotalBdm)
  }
  ...
  override protected def doAppend(
    ...
    val newAddressesForBdm = ArrayBuffer.empty[BigInt]
    val updatedBalanceAddresses = for ((addressId, balance) <- bdmBalances) yield {
      val kwbh = Keys.bdmBalanceHistory(addressId)
      val wbh  = rw.get(kwbh)
      if (wbh.isEmpty) {
        newAddressesForBdm += addressId
      }
      rw.put(Keys.bdmBalance(addressId)(height), balance)
      expiredKeys ++= updateHistory(rw, wbh, kwbh, balanceThreshold, Keys.bdmBalance(addressId))
      addressId
    }

    val changedAddresses = addressTransactions.keys ++ updatedBalanceAddresses

    if (newAddressesForBdm.nonEmpty) {
      val newSeqNr = rw.get(Keys.addressesForBdmSeqNr) + 1
      rw.put(Keys.addressesForBdmSeqNr, newSeqNr)
      rw.put(Keys.addressesForBdm(newSeqNr), newAddressesForBdm)
    }
    ...
    reward.foreach { lastReward =>
      ...
      rw.put(Keys.bdmAmount(height), bdmAmount(height - 1) + lastReward)
    }
      ...
      val discardedBlocks: Seq[Block] = for (currentHeight <- height until targetHeight by -1) yield {
        ...
        val discardedBlock = readWrite { rw =>
          ...
          for (aId <- rw.get(Keys.changedAddresses(currentHeight))) {
            ...
            balancesToInvalidate += (address -> Bdm)
            ...
            rw.delete(Keys.bdmBalance(addressId)(currentHeight))
            rw.filterHistory(Keys.bdmBalanceHistory(addressId), currentHeight)
          ...
          rw.delete(Keys.bdmAmount(currentHeight))
  ...
  override def balanceOnlySnapshots(address: Address, height: Int, assetId: Asset = Bdm): Option[(Int, Long)] = readOnly { db =>
    db.get(Keys.addressId(address)).flatMap { addressId =>
      assetId match {
        case Bdm =>
            closest(db.get(Keys.bdmBalanceHistory(addressId)), height).map { wh =>
              val b: Long = db.get(Keys.bdmBalance(addressId)(wh))
              (wh, b)
            }
  ...
  override def balanceSnapshots(address: Address, from: Int, to: BlockId): Seq[BalanceSnapshot] = readOnly { db =>
    db.get(Keys.addressId(address)).fold(Seq(BalanceSnapshot(1, 0, 0, 0))) { addressId =>
      ...
      val wbh      = slice(db.get(Keys.bdmBalanceHistory(addressId)), from, toHeigth)
      val lbh      = slice(db.get(Keys.leaseBalanceHistory(addressId)), from, toHeigth)
      for {
        (wh, lh) <- merge(wbh, lbh)
        wb = balanceAtHeightCache.get((wh, addressId), () => db.get(Keys.bdmBalance(addressId)(wh)))
