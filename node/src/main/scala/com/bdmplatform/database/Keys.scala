...
import com.bdmplatform.block.BlockHeader
import com.bdmplatform.common.state.ByteStr
import com.bdmplatform.common.utils.EitherExt2
import com.bdmplatform.lang.script.{Script, ScriptReader}
...
object Keys {
  ...
  def bdmBalanceHistory(addressId: BigInt): Key[Seq[Int]] = historyKey("bdm-balance-history", 5, addressId.toByteArray)
  def bdmBalance(addressId: BigInt)(height: Int): Key[Long] =
    Key("bdm-balance", hAddr(6, height, addressId), Option(_).fold(0L)(Longs.fromByteArray), Longs.toByteArray)
  ...
  val addressesForBdmSeqNr: Key[Int]                = intKey("addresses-for-bdm-seq-nr", 37)
  def addressesForBdm(seqNr: Int): Key[Seq[BigInt]] = Key("addresses-for-bdm", h(38, seqNr), readBigIntSeq, writeBigIntSeq)
  ...
  val bdmAmountPrefix: Short = 58
  def bdmAmount(height: Int): Key[BigInt] = Key("bdm-amount", h(bdmAmountPrefix, height), Option(_).fold(BigInt(0))(BigInt(_)), _.toByteArray)
}
