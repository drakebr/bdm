...
import com.bdmplatform.metrics.LevelDBStats
import com.bdmplatform.settings.DBSettings
...
import com.bdmplatform.utils.{ObservedLoadingCache, ScorexLogging}
...
abstract class Caches(spendableBalanceChanged: Observer[(Address, Asset)]) extends Blockchain with ScorexLogging {
  ...
  def append(diff: Diff, carryFee: Long, totalFee: Long, reward: Option[Long], block: Block): Unit = {
    ...
    val bdmBalances        = Map.newBuilder[BigInt, Long]
    ...
    for ((address, portfolioDiff) <- diff.portfolios) {
      ...
      if (portfolioDiff.balance != 0) {
        val wbalance = portfolioDiff.balance + balance(address, Bdm)
        bdmBalances += aid            -> wbalance
        newBalances += (address, Bdm) -> wbalance
      }
