...
import com.bdmplatform
import com.bdmplatform._
import com.bdmplatform.account.{Address, KeyPair, PublicKey}
...
import com.bdmplatform.history.{StorageFactory, randomSig}
...
import com.bdmplatform.lang.v1.compiler.{CompilerContext, ExpressionCompiler}
import com.bdmplatform.mining._
...
import com.bdmplatform.transaction.TxValidationError.SenderIsBlacklisted
...
import com.bdmplatform.transaction.{Asset, Transaction, _}
...
class UtxPoolSpecification
  ...
  private def mkBlockchain(senderAccount: Address, senderBalance: Long) = {
    ...
    val origSettings    = BdmSettings.fromRootConfig(config)
  ...
  private def transfer(sender: KeyPair, maxAmount: Long, time: Time) =
    ...
    } yield TransferTransactionV1.selfSigned(Bdm, sender, recipient, amount, time.getTimestamp(), Bdm, fee, Array.empty[Byte]).explicitGet())
  ...
  private def massTransferWithRecipients(sender: KeyPair, recipients: List[PublicKey], maxAmount: Long, time: Time) = {
    ...
    val txs = for { fee <- chooseNum(minFee, amount) } yield MassTransferTransaction
      .selfSigned(Bdm, sender, transfers, time.getTimestamp(), fee, Array.empty[Byte])
      .explicitGet()
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createBdmTransfer(sender, recipient, amountPart, fee, time.getTimestamp()).explicitGet()
  ...
  private def transactionV1Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransactionV1] = accountGen.map { recipient =>
    TransferTransactionV1.selfSigned(Bdm, sender, recipient, bdm(1), ts, Bdm, feeAmount, Array.emptyByteArray).explicitGet()
  }

  private def transactionV2Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransactionV2] = accountGen.map { recipient =>
    TransferTransactionV2.selfSigned(Bdm, sender, recipient, bdm(1), ts, Bdm, feeAmount, Array.emptyByteArray).explicitGet()
  }
  ...
  "UTX Pool" - {
    ...
    "correctly process constraints in packUnconfirmed" in {
      withDomain(bdmplatform.history.TransfersV2ActivatedAt0BdmSettings) { d =>
