...
import com.bdmplatform.account.{AddressOrAlias, Alias, KeyPair}
...
import com.bdmplatform.state.{Blockchain, LeaseBalance}
...
class LeasingExpirySpec extends FreeSpec with ScalaCheckPropertyChecks with WithDomain with Matchers with TransactionGen with NoShrink {
  ...
  private val genesis = for {
    ...
    genesisBlock = TestBlock.create(
      ts,
      Seq(
        GenesisTransaction.create(lessor, Constants.TotalBdm * Constants.UnitsInWave, ntpTime.getTimestamp()).explicitGet(),
        transfer,
        createAlias
      )
    )
