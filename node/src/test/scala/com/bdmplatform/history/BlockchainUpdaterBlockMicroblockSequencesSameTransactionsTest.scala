...
class BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest
      ...
      case (gen, rest) =>
        val finalMinerBalances = rest.map {
          case (a @ (bmb: BlockAndMicroblockSequence, last: Block)) =>
            withDomain(MicroblocksActivatedAt0BdmSettings) { d =>
  ...
  property("Miner fee from microblock [Genesis] <- [Empty] <~ (Micro with tx) <- [Empty]") {
    val preconditionsAndPayments: Gen[(KeyPair, GenesisTransaction, TransferTransactionV1, Int)] = for {
      ...
      payment: TransferTransactionV1 = createBdmTransfer(master, master, amt, fee, ts).explicitGet()
      ...
      for {
        ...
        microBlockTxs =
          (1 to txCount * microBlockCount)
            .map(step => createBdmTransfer(master, master, amt, fee, ts + step).explicitGet())
  ...
  def randomPayment(accs: Seq[KeyPair], ts: Long): Gen[TransferTransactionV1] =
    for {
      ...
      fee  <- smallFeeGen
      amt  <- smallFeeGen
    } yield createBdmTransfer(from, to, amt, fee, ts).explicitGet()
  ...
  def accsAndGenesis(): Gen[(Seq[KeyPair], KeyPair, Block, Int)] =
    for {
      ...
      miner   <- accountGen
      ts      <- positiveIntGen
      genesis1: GenesisTransaction = GenesisTransaction.create(alice, TOTAL_BDM / 4, ts).explicitGet()
      genesis2: GenesisTransaction = GenesisTransaction.create(bob, TOTAL_BDM / 4, ts + 1).explicitGet()
      genesis3: GenesisTransaction = GenesisTransaction.create(charlie, TOTAL_BDM / 4, ts + 2).explicitGet()
      genesis4: GenesisTransaction = GenesisTransaction.create(dave, TOTAL_BDM / 4, ts + 4).explicitGet()
    } yield
