...
import com.bdmplatform.transaction.assets.{IssueTransaction, SponsorFeeTransaction}
...
class BlockchainUpdaterSponsoredFeeBlockTest
  ...
  val sponsorPreconditions: Gen[Setup] = for {
    ...
    bdmFee                    = Sponsorship.toBdm(sponsorTx.minSponsoredAssetFee.get, sponsorTx.minSponsoredAssetFee.get)
    genesis: GenesisTransaction = GenesisTransaction.create(master, ENOUGH_AMT, ts).explicitGet()
    masterToAlice: TransferTransactionV1 = TransferTransactionV1
      .selfSigned(Bdm,
                  master,
                  alice,
                  feeAsset.fee + sponsorTx.fee + transferAssetBdmFee + bdmFee,
                  ts + 1,
                  Bdm,
                  transferAssetBdmFee,
  ...
  val SponsoredActivatedAt0BdmSettings: BdmSettings = settings.copy(blockchainSettings = SponsoredFeeActivatedAt0BlockchainSettings)

  property("not enough bdm to sponsor sponsored tx") {
    scenario(sponsorPreconditions, SponsoredActivatedAt0BdmSettings) {
      case (domain, (genesis, masterToAlice, feeAsset, sponsor, aliceToBob, bobToMaster, bobToMaster2)) =>
        ...
        domain.blockchainUpdater.processBlock(block4) should produce("negative bdm balance" /*"unavailable funds"*/ )
    }
  }
    ...
    scenario(sponsorPreconditions, SponsoredActivatedAt0BdmSettings) {
      case (domain, (genesis, masterToAlice, feeAsset, sponsor, aliceToBob, bobToMaster, bobToMaster2)) =>
...

        {
          ...
          val microBlocksBdmFee = microBlocks
            .flatMap(_.transactionData)
            .map(tx => Sponsorship.calcBdmFeeAmount(tx, ai => domain.blockchainUpdater.assetDescription(ai).map(_.sponsorship)))
            .sum

          domain.blockchainUpdater.totalFee(domain.blockchainUpdater.height) should contain(block0TotalFee + microBlocksBdmFee)
        }
    }
  }
