...
import com.bdmplatform.api.http.RewardApiRoute
...
import com.bdmplatform.settings.{Constants, FunctionalitySettings, RewardsSettings}
...
class BlockRewardSpec extends FreeSpec with ScalaCheckPropertyChecks with WithDomain with WithDB with Matchers with TransactionGen with NoShrink {
  ...
  private val genesis = for {
    ...
    genesisBlock = TestBlock.create(
      ...
      Seq(
        GenesisTransaction.create(sourceAddress, (Constants.TotalBdm - 60000) * Constants.UnitsInWave, ntpTime.getTimestamp()).explicitGet(),
  ...
  "Miner receives reward and fees" - {
    val ngEmptyScenario = for {
      ...
      tx1 = TransferTransactionV1
        .selfSigned(Bdm, issuer, sourceAddress, 10 * Constants.UnitsInWave, ntpTime.getTimestamp(), Bdm, OneTotalFee, Array.emptyByteArray)
        .explicitGet()
    ...
    "when all blocks without fees" in forAll(blockWithoutFeesScenario) {
      case (miner1, miner2, b1s, b2, b3s, b4) =>
        withDomain(rewardSettings) { d =>
          val initialBdmAmount = BigInt(Constants.TotalBdm) * BigInt(Constants.UnitsInWave)
          ...
          d.blockchainUpdater.bdmAmount(BlockRewardActivationHeight - 1) shouldBe initialBdmAmount
          ...
          d.blockchainUpdater.height shouldBe BlockRewardActivationHeight
          d.blockchainUpdater.bdmAmount(BlockRewardActivationHeight) shouldBe initialBdmAmount + InitialReward
          ...
          b3s.zipWithIndex.foreach {
            case (b, i) =>
              ...
              d.blockchainUpdater.bdmAmount(BlockRewardActivationHeight + i + 1) shouldBe initialBdmAmount + BigInt(InitialReward * (i + 2))
          ...
          d.blockchainUpdater.bdmAmount(BlockRewardActivationHeight + term) shouldBe initialBdmAmount + term * InitialReward + InitialReward + minIncrement
