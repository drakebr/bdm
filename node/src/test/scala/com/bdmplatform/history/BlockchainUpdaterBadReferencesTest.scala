...
class BlockchainUpdaterBadReferencesTest
  ...
  val preconditionsAndPayments: Gen[(GenesisTransaction, TransferTransactionV1, TransferTransactionV1, TransferTransactionV1)] = for {
    ...
    payment: TransferTransactionV1  <- bdmTransferGeneratorP(ts, master, recipient)
    payment2: TransferTransactionV1 <- bdmTransferGeneratorP(ts, master, recipient)
    payment3: TransferTransactionV1 <- bdmTransferGeneratorP(ts, master, recipient)
