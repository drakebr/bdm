...
class BlockchainUpdaterGeneratorFeeNextBlockOrMicroBlockTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    somePayment: TransferTransactionV1 = createBdmTransfer(sender, recipient, 1, 10, ts + 1).explicitGet()
    // generator has enough balance for this transaction if gets fee for block before applying it
    generatorPaymentOnFee: TransferTransactionV1 = createBdmTransfer(defaultSigner, recipient, 11, 1, ts + 2).explicitGet()
    someOtherPayment: TransferTransactionV1      = createBdmTransfer(sender, recipient, 1, 1, ts + 3).explicitGet()
