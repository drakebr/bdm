...
import com.bdmplatform.account.{KeyPair, Address, AddressOrAlias}
...
class BlockchainUpdaterMicroblockSunnyDayTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransactionV1 <- bdmTransferGeneratorP(ts, master, alice)
    aliceToBob  = createBdmTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts).explicitGet()
    aliceToBob2 = createBdmTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts + 1).explicitGet()
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) {
      case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
        ...
        val minerABalance = withDomain(MicroblocksActivatedAt0BdmSettings) { da =>
        ...
        val minerBBalance = withDomain(MicroblocksActivatedAt0BdmSettings) { db =>
