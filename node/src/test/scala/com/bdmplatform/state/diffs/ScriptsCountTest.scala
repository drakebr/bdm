  ...
  property("check pre-Ride4DApps scripts run count") {
    forAll(for {
      ...
      tr1 = TransferTransactionV2
        .selfSigned(Bdm, master, acc, 10000000000L, timestamp, Bdm, fee, ByteStr(Array()))
        .explicitGet()
      tr2 = TransferTransactionV2
        .selfSigned(IssuedAsset(issueScr.id()), master, acc, 1000000000L, timestamp, Bdm, fee, ByteStr(Array()))
        .explicitGet()
      mt1 = MassTransferTransaction.selfSigned(Bdm, master, List(ParsedTransfer(acc, 1)), timestamp, fee, ByteStr(Array())).explicitGet()
