...
import com.bdmplatform.account.{AddressScheme, KeyPair, PrivateKey, PublicKey}
...
import com.bdmplatform.state.diffs.ExchangeTransactionDiff.getOrderFeePortfolio
...
import com.bdmplatform.transaction.assets.exchange.OrderOps._
import com.bdmplatform.transaction.assets.exchange.{Order, _}
import com.bdmplatform.transaction.assets.{IssueTransaction, IssueTransactionV1, IssueTransactionV2}
...
class ExchangeTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with Inside with NoShrink {
  ...
  property("Preserves bdm invariant, stores match info, rewards matcher") {
  ...
  property("Preserves assets invariant (matcher's fee in one of the assets of the pair or in Bdm), stores match info, rewards matcher") {
            ...
            val restoredMatcherPortfolio =
              Monoid.combineAll(
                Seq(
                  ...
                  ExchangeTransactionDiff.bdmPortfolio(-exchange.fee)
                )
              )
  ...
  property("buy bdm without enough money for fee") {
    val preconditions: Gen[(GenesisTransaction, GenesisTransaction, IssueTransactionV1, ExchangeTransaction)] = for {
      ...
      exchange <- Gen.oneOf(
        exchangeV1GeneratorP(buyer, seller, Bdm, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000)),
        exchangeV2GeneratorP(buyer, seller, Bdm, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000))
      )
    } yield {
  ...
  property("small fee cases") {
    ...
    forAll(preconditions, priceGen) {
      case ((buyer, seller, matcher, gen1, gen2, issue1), price) =>
        val assetPair = AssetPair(IssuedAsset(issue1.id()), Bdm)
  ...
  property("Diff for ExchangeTransaction works as expected and doesn't use rounding inside") {
    ...
    val assetPair                                          = AssetPair(Bdm, IssuedAsset(issue1.id()))
  ...
  property("Disable use OrderV1 on SmartAccount") {
    ...
    forAll(for {
      ...
      tr1     = createBdmTransfer(MATCHER, buyer.toAddress, Long.MaxValue / 3, enoughFee, ts + 1).explicitGet()
      tr2     = createBdmTransfer(MATCHER, seller.toAddress, Long.MaxValue / 3, enoughFee, ts + 2).explicitGet()
