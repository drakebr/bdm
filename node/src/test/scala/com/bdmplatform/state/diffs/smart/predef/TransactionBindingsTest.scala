package com.bdmplatform.state.diffs.smart.predef
...
import com.bdmplatform.common.state.diffs.ProduceError._
import com.bdmplatform.lang.Testing.evaluated
...
import com.bdmplatform.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_BYTESTR, CONST_LONG, CONST_STRING, EVALUATED}
...
import com.bdmplatform.transaction.assets.exchange.{Order, OrderType}
import com.bdmplatform.transaction.smart.BlockchainContext.In
import com.bdmplatform.transaction.smart.BdmEnvironment
import com.bdmplatform.transaction.{Proofs, ProvenTransaction, VersionedTransaction}
...
class TransactionBindingsTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
    ...
    forAll(invokeScriptGen) { t =>
      ...
      val script =
        s"""
            ...
            |     then if (${t.payment.headOption.exists(_.assetId != Bdm)})
  ...
  property("MassTransferTransaction binding") {
    forAll(massTransferGen) { t =>
      ...
      val script = s"""
                      ...
                      |    let assetId = if (${t.assetId != Bdm}) then extract(t.assetId) == base58'${t.assetId.maybeBase58Repr
                        .getOrElse("")}'
                      |      else isDefined(t.assetId) == false
    ...
    forAll(exchangeTransactionGen) { t =>
      def pg(ord: Order) = {
        ...
        val script = s"""
           ...
           |   let ${oType}AssetPairAmount = if (${ord.assetPair.amountAsset != Bdm}) then extract(t.${oType}Order.assetPair.amountAsset) == base58'${ord.assetPair.amountAsset.maybeBase58Repr
                          .getOrElse("")}'
           |   else isDefined(t.${oType}Order.assetPair.amountAsset) == false
           |   let ${oType}AssetPairPrice = if (${ord.assetPair.priceAsset != Bdm}) then extract(t.${oType}Order.assetPair.priceAsset) == base58'${ord.assetPair.priceAsset.maybeBase58Repr
                          .getOrElse("")}'
           |   else isDefined(t.${oType}Order.assetPair.priceAsset) == false
           |   let ${oType}MatcherFeeAssetId = if (${ord.matcherFeeAssetId != Bdm}) then extract(t.${oType}Order.matcherFeeAssetId) == base58'${ord.matcherFeeAssetId.maybeBase58Repr
                          .getOrElse("")}'
  ...
  property("Order binding") {
    forAll(orderGen) { t =>
      val s = s"""
                 ...
                 |   ${Range(0, 8).map(letProof(t.proofs, "t")).mkString("\n")}
                 |   let assetPairAmount = if (${t.assetPair.amountAsset != Bdm}) then extract(t.assetPair.amountAsset) == base58'${t.assetPair.amountAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.amountAsset) == false
                 |   let assetPairPrice = if (${t.assetPair.priceAsset != Bdm}) then extract(t.assetPair.priceAsset) == base58'${t.assetPair.priceAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.priceAsset) == false
                 |   let matcherFeeAssetId = if (${t.matcherFeeAssetId != Bdm}) then extract(t.matcherFeeAssetId) == base58'${t.matcherFeeAssetId.maybeBase58Repr
                   .getOrElse("")}'
  ...
  def runForAsset(script: String): Either[String, EVALUATED] = {
    ...
    import com.bdmplatform.lang.v1.CTX._
    ...
    val ctx =
      ...
      BdmContext.build(DirectiveSet(V2, Asset, Expression).explicitGet())

    val environment = new BdmEnvironment(chainId, Coeval(???), null, EmptyBlockchain, Coeval(null))
    for {
  ...
  def runWithSmartTradingActivated(script: String, t: In = null, chainId: Byte = chainId): Either[String, EVALUATED] = {
    ...
    val Success(expr, _) = Parser.parseExpr(script)
    val environment = new BdmEnvironment(chainId, Coeval(t), null, EmptyBlockchain, Coeval(null))
