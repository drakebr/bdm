...
import com.bdmplatform.transaction.smart.BlockchainContext
...
package object predef {
  ...
  def scriptWithAllV1Functions(tx: DataTransaction, t: TransferTransaction): String =
    s"""${dropLastLine(scriptWithV1PureFunctions(tx, t))}
       |${dropLastLine(scriptWithV1BdmFunctions(tx, t))}
       |${dropLastLine(scriptWithCryptoFunctions)}
       |if rnd then pure && bdm else crypto""".stripMargin
  ...
  def scriptWithV1BdmFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s""" # Bdm context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && bdmBalance(tx.sender) != 0
       |
       | let bdm = txById && entries && balances && aFromPK && aFromStrOrRecip && height > 0
       | bdm""".stripMargin
