...
import com.bdmplatform.account.{Address, AddressScheme, Alias, KeyPair}
...
import com.bdmplatform.lang.contract.DApp.{CallableAnnotation, CallableFunction}
import com.bdmplatform.lang.directives.DirectiveSet
import com.bdmplatform.lang.directives.values.{DApp => DAppType, _}
import com.bdmplatform.lang.script.ContractScript
import com.bdmplatform.lang.script.v1.ExprScript
import com.bdmplatform.lang.v1.FunctionHeader.{Native, User}
import com.bdmplatform.lang.v1.compiler.Terms
import com.bdmplatform.lang.v1.compiler.Terms._
import com.bdmplatform.lang.v1.evaluator.FunctionIds.{CREATE_LIST, THROW}
import com.bdmplatform.lang.v1.evaluator.ctx.impl.bdm.{FieldNames, BdmContext}
import com.bdmplatform.lang.v1.evaluator.ctx.impl.{CryptoContext, PureContext}
import com.bdmplatform.lang.v1.evaluator.{FunctionIds, ScriptResult}
import com.bdmplatform.lang.v1.parser.{Expressions, Parser}
import com.bdmplatform.lang.v1.traits.Environment
import com.bdmplatform.lang.v1.{ContractLimits, FunctionHeader, compiler}
...
import com.bdmplatform.state.utils._
import com.bdmplatform.transaction.Asset.{IssuedAsset, Bdm}
import com.bdmplatform.transaction.TxValidationError.TransactionNotAllowedByScript
import com.bdmplatform.transaction.assets._
import com.bdmplatform.transaction.smart.InvokeScriptTransaction.Payment
import com.bdmplatform.transaction.smart.script.trace.{AssetVerifierTrace, InvokeScriptTrace}
...
import com.bdmplatform.transaction.{Asset, _}
import com.bdmplatform.{NoShrink, TransactionGen, WithDB}
...
class InvokeScriptTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithDB with Inside {
  ...
  def paymentContract(senderBinding: String,
                      ...
                      assets: List[Asset] = List(Bdm)): DApp = {
  ...
  def paymentContractGen(address: Address, amount: Long, assets: List[Asset] = List(Bdm))(func: String) =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
    } yield paymentContract(senderBinging, argBinding, func, address, amount, assets)

  def defaultPaymentContractGen(address: Address, amount: Long, assets: List[Asset] = List(Bdm))(someName: String) =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
    ...
    forAll(for {
      ...
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffAndState(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
          case (blockDiff, newState) =>
            blockDiff.scriptsRun shouldBe 1
            newState.balance(acc, Bdm) shouldBe amount
        }
    }
  }
  ...
  property("can't make more than 10 payments") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = (paymentContractGen(a, am, List.fill(11)(Bdm)) _)
      r <- preconditionsAndSetContract(contractGen)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
      ...
      case (acc, amount, genesis, setScript, ci, asset, master, ts) =>
        val t =
          TransferTransactionV2
            .selfSigned(IssuedAsset(asset.id()), master, acc, asset.quantity / 10, ts, Bdm, enoughFee, Array[Byte]())
            .explicitGet()
  ...
  property("payment should be positive") {
    forAll(for {
      ...
      ci = InvokeScriptTransaction.selfSigned(invoker, master, Some(fc), Seq(Payment(-1, Bdm)), fee, Bdm, ts)
  ...
  property("can't overflow payment + fee") {
    forAll(for {
      ...
      r <- preconditionsAndSetContract(contractGen,
                                       invokerGen = Gen.oneOf(Seq(invoker)),
                                       payment = Some(Payment(Long.MaxValue, Bdm)),
                                       feeGen = ciFee(1))
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("can't overflow sum of payment in contract") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = (paymentContractGen(a, Long.MaxValue / 2 + 2, List.fill(4)(Bdm)) _)
      invoker <- accountGen
      ts      <- timestampGen
      r       <- preconditionsAndSetContract(contractGen, invokerGen = Gen.oneOf(Seq(invoker)), payment = Some(Payment(1, Bdm)), feeGen = ciFee(1))
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
          _ should produce("Attempt to transfer unavailable funds")
  ...
  property("invoking contract with sponsored fee") {
    forAll(for {
      ...
      case (ts, acc, amount, genesis, setScript, ci, master, sponsoredAsset, setSponsorship) =>
        val t =
          TransferTransactionV2
            .selfSigned(IssuedAsset(sponsoredAsset.id()), master, ci.sender, sponsoredAsset.quantity / 10, ts, Bdm, enoughFee, Array[Byte]())
            .explicitGet()
  ...
  property("Default function invocation should produce error if contract default function has arguments") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "default", a, am, List(Bdm))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("Default function invocation should produce error if contract does't have default function") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "undefault", a, am, List(Bdm))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
