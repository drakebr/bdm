...
import com.bdmplatform.transaction.{Asset, GenesisTransaction, TxValidationError}
...
class TransferTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
  ...
  property("transfers assets to recipient preserving bdm invariant") {
          ...
          case (totalDiff, newState) =>
            ...
            if (transfer.sender.toAddress != recipient) {
              transfer.assetId match {
                ...
                case Bdm                => recipientPortfolio shouldBe Portfolio(transfer.amount, LeaseBalance.empty, Map.empty)
              }
            }
        }
    }
  ...
  property("handle transactions with amount + fee > Long.MaxValue") {
    val precs = for {
      ...
      transfer = TransferTransactionV1.selfSigned(asset, master, recepient, Long.MaxValue, ts, Bdm, 100000, Array.emptyByteArray).explicitGet()
