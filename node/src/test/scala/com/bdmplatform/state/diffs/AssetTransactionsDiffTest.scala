...
class AssetTransactionsDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithDB {
  ...
  property("Issue+Reissue+Burn do not break bdm invariant and updates state") {
  ...
  property("Can burn non-owned alias if feature 'BurnAnyTokens' activated") {
    val setup = for {
      ...
      assetTransfer <- transferGeneratorP(issuer, burner, IssuedAsset(issue.assetId), Bdm)
      bdmTransfer <- bdmTransferGeneratorP(issuer, burner)
      burn = BurnTransactionV1.selfSigned(burner, IssuedAsset(issue.assetId), assetTransfer.amount, bdmTransfer.amount, timestamp).explicitGet()
    } yield (genesis, issue, assetTransfer, bdmTransfer, burn)
    ...
    forAll(setup) {
      case (genesis, issue, assetTransfer, bdmTransfer, burn) =>
        assertDiffAndState(Seq(TestBlock.create(Seq(genesis, issue, assetTransfer, bdmTransfer))), TestBlock.create(Seq(burn)), fs) {
          case (_, newState) =>
  ...
  property("Can not total issue > long.max") {
    val setup = for {
      ...
      transfer = TransferTransactionV1.selfSigned(assetId, issuer, holder, quantity - 1, timestamp, Bdm, fee, attachment).explicitGet()
  ...
  def genesisIssueTransferReissue(code: String): Gen[(Seq[GenesisTransaction], IssueTransactionV2, TransferTransactionV1, ReissueTransactionV1)] =
    for {
      ...
      timestamp          <- timestampGen
      initialBdmAmount <- Gen.choose(Long.MaxValue / 1000, Long.MaxValue / 100)
      accountA           <- accountGen
      accountB           <- accountGen
      smallFee           <- Gen.choose(1l, 10l)
      genesisTx1 = GenesisTransaction.create(accountA, initialBdmAmount, timestamp).explicitGet()
      genesisTx2 = GenesisTransaction.create(accountB, initialBdmAmount, timestamp).explicitGet()
      reissuable = true
      ...
      transfer = TransferTransactionV1
        .selfSigned(assetId, accountA, accountB, issue.quantity, timestamp + 2, Bdm, smallFee, Array.empty)
        .explicitGet()
