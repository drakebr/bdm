...
import com.bdmplatform.lagonaki.mocks.TestBlock.{create => block}
...
import com.bdmplatform.transaction.assets.{IssueTransactionV1, SponsorFeeTransaction}
...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("not enough fee") {
    ...
    val setup = for {
      ...
      assetOverspend = TransferTransactionV1
        .selfSigned(Bdm, master, recipient.toAddress, 1000000, ts + 1, IssuedAsset(assetId), issueTx.quantity + 1, Array.emptyByteArray)
        .right
        .get
      ...
      bdmOverspend = TransferTransactionV1
        .selfSigned(Bdm, master, recipient.toAddress, 1000000, ts + 3, IssuedAsset(assetId), fee, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issueTx, sponsorTx, assetOverspend, insufficientFee, bdmOverspend)

    forAll(setup) {
      case (genesis, issue, sponsor, assetOverspend, insufficientFee, bdmOverspend) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          ...
          val expectedError =
            s"Fee for TransferTransaction (${insufficientFee.fee} in ${issue.assetId.base58})" ++
              s" does not exceed minimal value of 100000 BDM or $minFee ${issue.assetId.base58}"

          blockDiffEi should produce(expectedError)
        }
        assertDiffEi(setupBlocks, block(Seq(bdmOverspend)), s) { blockDiffEi =>
          if (bdmOverspend.fee > issue.quantity)
            blockDiffEi should produce("unavailable funds")
          else
            blockDiffEi should produce("negative bdm balance")
        }
    }
  }

  property("not enough bdm to pay fee after leasing") {
    val s = settings(0)
    val setup = for {
      master <- accountGen
      alice  <- accountGen
      ...
      transferAssetTx: TransferTransactionV1 = TransferTransactionV1
        .selfSigned(IssuedAsset(assetId), master, alice.toAddress, issueTx.quantity, ts + 2, Bdm, fee, Array.emptyByteArray)
        .right
        .get
  ...
  property("cannot cancel sponsorship") {
    ...
    forAll(setup) {
      case (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("(99999999 in BDM) does not exceed minimal value of 100000000 BDM")
        }
    }
  }
  ...
  property("sponsor has no BDM but receives them just in time") {
    val s = settings(0)
    val setup = for {
      master    <- accountGen
      recipient <- accountGen
      ...
      assetTransfer = TransferTransactionV1
        .selfSigned(assetId, master, recipient, issue.quantity, ts + 3, Bdm, 100000, Array.emptyByteArray)
        .right
        .get
      bdmTransfer = TransferTransactionV1
        .selfSigned(Bdm, master, recipient, 99800000, ts + 4, Bdm, 100000, Array.emptyByteArray)
        .right
        .get
      backBdmTransfer = TransferTransactionV1
        .selfSigned(Bdm, recipient, master, 100000, ts + 5, assetId, 100, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issue, sponsor, assetTransfer, bdmTransfer, backBdmTransfer)

    forAll(setup) {
      case (genesis, issue, sponsor, assetTransfer, bdmTransfer, backBdmTransfer) =>
        assertDiffAndState(Seq(block(Seq(genesis, issue, sponsor, assetTransfer, bdmTransfer))), block(Seq(backBdmTransfer)), s) {
          case (_, state) =>
