...
import com.bdmplatform.account.{KeyPair, AddressOrAlias}
import com.bdmplatform.common.utils.EitherExt2
import com.bdmplatform.lagonaki.mocks.TestBlock
import com.bdmplatform.lang.v1.compiler.Terms.{CONST_BYTESTR, CaseObj}
import com.bdmplatform.state.diffs._
import com.bdmplatform.state.diffs.smart.predef._
import com.bdmplatform.transaction.Asset.Bdm
import com.bdmplatform.transaction.transfer._
import com.bdmplatform.transaction.{CreateAliasTransaction, GenesisTransaction}
...
class AddressFromRecipientScenarioTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
  ...
  val preconditionsAndAliasCreations: Gen[(Seq[GenesisTransaction], CreateAliasTransaction, TransferTransactionV1, TransferTransactionV1)] = for {
    ...
    transferViaAddress <- transferGeneratorP(master, other, Bdm, Bdm)
    transferViaAlias   <- transferGeneratorP(master, AddressOrAlias.fromBytes(alias.bytes.arr, 0).explicitGet()._1, Bdm, Bdm)
