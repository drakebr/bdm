...
import com.bdmplatform.lang.{Global, Testing}
  ...
  val preconditions: Gen[
    ...
    for {
      ...
      transferFromCompanyToA = TransferTransactionV1
        .selfSigned(assetId, company, accountA, 1, ts + 20, Bdm, 1000, Array.empty)
        .explicitGet()

      transferFromAToB = TransferTransactionV1
        .selfSigned(assetId, accountA, accountB, 1, ts + 30, Bdm, 1000, Array.empty)
        .explicitGet()
