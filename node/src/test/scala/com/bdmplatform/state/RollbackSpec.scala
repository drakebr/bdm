...
import com.bdmplatform.account.{Address, KeyPair}
...
import com.bdmplatform.features.BlockchainFeatures._
import com.bdmplatform.features._
import com.bdmplatform.lagonaki.mocks.TestBlock
import com.bdmplatform.lang.script.v1.ExprScript
import com.bdmplatform.lang.v1.compiler.Terms.TRUE
import com.bdmplatform.settings.{TestFunctionalitySettings, BdmSettings}
import com.bdmplatform.state.reader.LeaseDetails
import com.bdmplatform.transaction.Asset.{IssuedAsset, Bdm}
import com.bdmplatform.transaction.TxValidationError.AliasDoesNotExist
import com.bdmplatform.transaction.assets.{IssueTransactionV1, ReissueTransactionV1}
...
import com.bdmplatform.transaction.{CreateAliasTransactionV1, DataTransaction, GenesisTransaction, Transaction}
import com.bdmplatform.{NoShrink, TestTime, TransactionGen, history}
...
class RollbackSpec extends FreeSpec with Matchers with WithDomain with TransactionGen with PropertyChecks with NoShrink {
  ...
  private def transfer(sender: KeyPair, recipient: Address, amount: Long) =
    TransferTransactionV1.selfSigned(Bdm, sender, recipient, amount, nextTs, Bdm, 1, Array.empty[Byte]).explicitGet()
      ...
      case 2 =>
        List(
          MassTransferTransaction
            .selfSigned(Bdm, sender, List(ParsedTransfer(recipient, amount), ParsedTransfer(recipient, amount)), nextTs, 10000, Array.empty[Byte])
            .explicitGet())
      case _ => List(TransferTransactionV1.selfSigned(Bdm, sender, recipient, amount, nextTs, Bdm, 1000, Array.empty[Byte]).explicitGet())
    }
  }
  ...
  "Rollback resets" - {
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender, com.bdmplatform.state.diffs.ENOUGH_AMT))
    ...
    "bdm balances" in forAll(accountGen, positiveLongGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
            ...
            TestBlock.create(
              ...
              Seq(
                TransferTransactionV1
                  .selfSigned(IssuedAsset(issueTransaction.id()), sender, recipient, assetAmount, nextTs, Bdm, 1, Array.empty[Byte])
                  .explicitGet())
            ))
    ...
    def createSettings(preActivatedFeatures: (BlockchainFeature, Int)*): BdmSettings = {
      ...
      history.DefaultBdmSettings.copy(blockchainSettings = history.DefaultBdmSettings.blockchainSettings.copy(functionalitySettings = tfs))
    }
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender, com.bdmplatform.state.diffs.ENOUGH_AMT))
