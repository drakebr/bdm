...
import com.bdmplatform.settings.{NetworkSettings, loadConfig}
...
class BlacklistParallelSpecification extends FeatureSpec with GivenWhenThen with ParallelTestExecution {
  ...
  private val config = loadConfig(ConfigFactory.parseString("""bdm.network {
      |  known-peers = []
      |  file = null
      |  black-list-residence-time: 1s
      |}""".stripMargin))

  private val networkSettings = config.as[NetworkSettings]("bdm.network")
