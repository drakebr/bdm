...
import com.bdmplatform.block.{Block, SignerData}
...
import com.bdmplatform.metrics.Instrumented
...
class BlockSpecification extends PropSpec with PropertyChecks with TransactionGen with Matchers with NoShrink {
  ...
  val blockGen = for {
    ...
    paymentTransaction        <- bdmTransferGeneratorP(time, sender, recipient)
    transferTrancation        <- transferGeneratorP(1 + time, sender, recipient, assetId, Bdm)
    anotherPaymentTransaction <- bdmTransferGeneratorP(2 + time, sender, recipient)
  ...
  def bigBlockGen(amt: Int): Gen[Block] =
    for {
      ...
      sender                                    <- accountGen
      recipient                                 <- accountGen
      paymentTransaction: TransferTransactionV1 <- bdmTransferGeneratorP(time, sender, recipient)
    } yield
      Block
        .buildAndSign(3,
                      time,
