...
import com.bdmplatform.RequestGen
...
import com.bdmplatform.transaction.{Asset, Proofs, Transaction}
  ...
  "returns appropriate error code when validation fails for" - {
    ...
    "transfer transaction" in {
      forAll(broadcastTransferReq) { tr =>
        ...
        forAll(nonPositiveLong) { q =>
          posting(tr.copy(amount = q)) should produce(NonPositiveAmount(s"$q of ${tr.assetId.getOrElse("bdm")}"))
        }
        forAll(invalidBase58) { pk =>
