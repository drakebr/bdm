...
import com.bdmplatform.database.{Keys, LevelDBWriter}
import com.bdmplatform.db.DBCacheSettings
...
import com.bdmplatform.transaction.{Asset, BlockchainUpdater, GenesisTransaction, Transaction}
    ...
    val bps: Seq[BlockProducer] = Seq(
      (ts, reference, account) => {
        ...
        val tx1 = TransferTransactionV2
          .selfSigned(Bdm, account, recipient1, 10 * Constants.UnitsInWave, ts, Bdm, 400000, Array())
          .explicitGet()
        val tx2 = TransferTransactionV2
          .selfSigned(Bdm, account, recipient2, 5 * Constants.UnitsInWave, ts, Bdm, 400000, Array())
          .explicitGet()
...
object MiningWithRewardSuite {
  ...
  val commonSettings: BdmSettings                    = BdmSettings.fromRootConfig(loadConfig(ConfigFactory.load()))
  ...
  val settings: BdmSettings = commonSettings.copy(minerSettings = minerSettings, blockchainSettings = blockchainSettings)
