...
import com.bdmplatform.transaction.assets.exchange.{AssetPair, Order, OrderType, _}
import com.bdmplatform.transaction.smart.Verifier
  ...
  property("AssetPair test") {
    forAll(assetIdGen, assetIdGen) { (assetA, assetB) =>
      whenever(assetA != assetB) {
        val assetAId: Asset = assetA.fold[Asset](Bdm)(arr => IssuedAsset(arr))
        val assetBId: Asset = assetB.fold[Asset](Bdm)(arr => IssuedAsset(arr))
