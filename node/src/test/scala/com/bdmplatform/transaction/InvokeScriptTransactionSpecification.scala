...
import com.bdmplatform.api.http.{InvokeScriptRequest, SignedInvokeScriptRequest}
import com.bdmplatform.common.state.ByteStr
import com.bdmplatform.common.utils.{Base64, _}
import com.bdmplatform.lang.v1.compiler.Terms
import com.bdmplatform.lang.v1.compiler.Terms.{ARR, CONST_LONG, CaseObj}
import com.bdmplatform.lang.v1.compiler.Types.CASETYPEREF
import com.bdmplatform.lang.v1.{ContractLimits, FunctionHeader, Serde}
import com.bdmplatform.protobuf.transaction._
import com.bdmplatform.protobuf.{Amount, transaction}
import com.bdmplatform.serialization.Deser
import com.bdmplatform.transaction.Asset.{IssuedAsset, Bdm}
import com.bdmplatform.transaction.TxValidationError.NonPositiveAmount
...
import com.bdmplatform.{TransactionGen, crypto}
...
class InvokeScriptTransactionSpecification extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("Signed InvokeScriptTransactionRequest parser") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(1, Bdm))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
  ...
  property("can't have zero amount") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(0, Bdm))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs = List("CC1jQ4qkuVfMvB2Kpg2Go6QKXJxUFC8UUswUxBsxwisrR8N5s3Yc8zA6dhjTwfWKfdouSTAnRXCxTXb3T6pJq3T")
    )
    req.toTx shouldBe Left(NonPositiveAmount(0, "Bdm"))
    AddressScheme.current = DefaultAddressScheme
  }
  ...
  property("can't have negative amount") {
    ...
    req.toTx shouldBe Left(NonPositiveAmount(-1, "Bdm"))
    AddressScheme.current = DefaultAddressScheme
  }
}
