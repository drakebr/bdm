...
import com.bdmplatform.features.BlockchainFeatureStatus
import com.bdmplatform.features.FeatureProvider._
import com.bdmplatform.history
...
class BlockchainUpdaterTest extends FreeSpec with Matchers with HistoryTest with ShouldVerb with WithDomain {
  ...
  private val BdmSettings = history.DefaultBdmSettings.copy(
    blockchainSettings = history.DefaultBdmSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultBdmSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultBdmSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val BdmSettingsWithDoubling = BdmSettings.copy(
    blockchainSettings = BdmSettings.blockchainSettings.copy(
      functionalitySettings = BdmSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(BdmSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(BdmSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(BdmSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(BdmSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(BdmSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(BdmSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(BdmSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(BdmSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(BdmSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
