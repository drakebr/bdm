...
import com.bdmplatform.account.{PublicKey, Address}
  ...
  property("VersionedTransferTransactionSpecification id doesn't depend on proof") {
    forAll(accountGen, accountGen, proofsGen, proofsGen, bytes32gen) {
      case (acc1, acc2, proofs1, proofs2, attachment) =>
        val tx1 = TransferTransactionV2.create(Bdm, acc2, acc2.toAddress, 1, 1, Bdm, 1, attachment, proofs1).explicitGet()
        val tx2 = TransferTransactionV2.create(Bdm, acc2, acc2.toAddress, 1, 1, Bdm, 1, attachment, proofs2).explicitGet()
