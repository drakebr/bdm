...
import com.bdmplatform.settings.{Constants, TestFunctionalitySettings}
 ...
 "minSponsoredAssetFee": null
  ...
  property("miner receives one satoshi less than sponsor pays") {
    val setup = for {
      ...
      transfer = TransferTransactionV1.selfSigned(Bdm, acc, acc, 1, ts, feeAssetId = IssuedAsset(issue.id()), minFee, Array()).explicitGet()
    ...
    forAll(setup) {
      case (acc, genesis, issue, sponsor, transfer) =>
        ...
        assertNgDiffState(Seq(b0, b1), b2, NgAndSponsorshipSettings) {
          case (_, state) =>
            state.balance(acc, Bdm) - ENOUGH_AMT shouldBe 0
        }
    }
  }
  ...
  property("miner receives one satoshi more than sponsor pays") {
    val setup = for {
      ...
      sponsor   = SponsorFeeTransaction.selfSigned(acc, IssuedAsset(issue.id()), Some(minFee), One, ts).explicitGet()
      transfer1 = TransferTransactionV1.selfSigned(Bdm, acc, acc, 1, ts, feeAssetId = IssuedAsset(issue.id()), minFee + 7, Array()).explicitGet()
      transfer2 = TransferTransactionV1.selfSigned(Bdm, acc, acc, 1, ts, feeAssetId = IssuedAsset(issue.id()), minFee + 9, Array()).explicitGet()
  ...
  property("sponsorship changes in the middle of a block") {
    val setup = for {
      ...
      transfer1 = TransferTransactionV1.selfSigned(Bdm, acc, acc, 1, ts, IssuedAsset(issue.id()), feeAmount = minFee, Array()).explicitGet()
      sponsor2  = SponsorFeeTransaction.selfSigned(acc, IssuedAsset(issue.id()), Some(minFee * 10), One, ts).explicitGet()
      transfer2 = TransferTransactionV1.selfSigned(Bdm, acc, acc, 1, ts, IssuedAsset(issue.id()), feeAmount = minFee * 10, Array()).explicitGet()
