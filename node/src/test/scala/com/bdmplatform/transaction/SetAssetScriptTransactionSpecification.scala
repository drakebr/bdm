...
class SetAssetScriptTransactionSpecification extends GenericTransactionSpecification[SetAssetScriptTransaction] {
  def transactionParser: com.bdmplatform.transaction.TransactionParserFor[SetAssetScriptTransaction] = SetAssetScriptTransaction
  def updateProofs(tx: SetAssetScriptTransaction, p: Proofs): SetAssetScriptTransaction = {
    tx.copy(proofs = p)
  }
  def generator: Gen[((Seq[com.bdmplatform.transaction.Transaction], SetAssetScriptTransaction))] = setAssetScriptTransactionGen
