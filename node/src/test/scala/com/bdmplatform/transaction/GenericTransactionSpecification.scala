...
abstract class GenericTransactionSpecification[T <: com.bdmplatform.transaction.Transaction]
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen {

  def transactionParser: com.bdmplatform.transaction.TransactionParserFor[T]
  def updateProofs(tx: T, p: Proofs): T
  def generator: Gen[((Seq[com.bdmplatform.transaction.Transaction], T))]
