...
import com.bdmplatform.lang.v1.testing.{ScriptGen, TypedScriptGen}
import com.bdmplatform.lang.v1.{ContractLimits, FunctionHeader}
import com.bdmplatform.settings.{Constants, FunctionalitySettings}
import com.bdmplatform.state._
import com.bdmplatform.state.diffs.ENOUGH_AMT
import com.bdmplatform.transaction.Asset.{IssuedAsset, Bdm}
import com.bdmplatform.transaction._
import com.bdmplatform.transaction.assets._
import com.bdmplatform.transaction.assets.exchange._
import com.bdmplatform.transaction.lease._
import com.bdmplatform.transaction.smart.{InvokeScriptTransaction, SetScriptTransaction}
import com.bdmplatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, ParsedTransfer}
...
trait TransactionGenBase extends ScriptGen with TypedScriptGen with NTPTime { _: Suite =>
  ...
  protected def bdm(n: Float): Long = (n * 100000000L).toLong
  ...
  val bdmAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]]    = Gen.frequency((1, bdmAssetGen), (10, Gen.option(bytes32gen.map(ByteStr(_)))))

  val assetPairGen = assetIdGen.flatMap {
    case None => bytes32gen.map(b => AssetPair(Bdm, IssuedAsset(ByteStr(b))))
    case Some(a1bytes) =>
      ...
      Gen.oneOf(Gen.const(None), a2bytesGen).map { a2 =>
        ...
        val asset2 = a2.fold[Asset](Bdm)(arr => IssuedAsset(ByteStr(arr)))
  ...
  def transferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransactionV1] =
    ...
    } yield TransferTransactionV1.selfSigned(Bdm, sender, recipient, amount, timestamp, Bdm, feeAmount, attachment).explicitGet()

  def transferGeneratorPV2(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransactionV2] =
    for {
      amount                                    <- Gen.choose(1, maxAmount)
      (_, _, _, _, _, _, feeAmount, attachment) <- transferParamGen
    } yield TransferTransactionV2.selfSigned(Bdm, sender, recipient, amount, timestamp, Bdm, feeAmount, attachment).explicitGet()
  ...
  def bdmTransferGeneratorP(sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(sender, recipient, Bdm, Bdm)

  def bdmTransferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(timestamp, sender, recipient, Bdm, Bdm)
  ...
  def createBdmTransfer(
    ...
    TransferTransactionV1.selfSigned(Bdm, sender, recipient, amount, timestamp, Bdm, fee, Array())
  ...
  def versionedTransferGenP(sender: PublicKey, recipient: Address, proofs: Proofs): Gen[TransferTransactionV2] =
    (for {
      ...
      fee       <- smallFeeGen
      timestamp <- timestampGen
    } yield TransferTransactionV2.create(Bdm, sender, recipient, amt, timestamp, Bdm, fee, Array.emptyByteArray, proofs).explicitGet())
      .label("VersionedTransferTransactionP")

  val transferWithBdmFeeGen = for {
    (assetId, sender, recipient, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransactionV1.selfSigned(assetId, sender, recipient, amount, timestamp, Bdm, feeAmount, attachment).explicitGet()

  val selfTransferWithBdmFeeGen: Gen[TransferTransactionV1] = for {
    (assetId, sender, _, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransactionV1.selfSigned(assetId, sender, sender, amount, timestamp, Bdm, feeAmount, attachment).explicitGet()
  ...
  val invokeScriptGen = for {
    ...
    fee       <- smallFeeGen
    timestamp <- timestampGen
  } yield InvokeScriptTransaction.selfSigned(sender, dappAddress.toAddress, Some(fc), po.toSeq, fee, Bdm, timestamp).explicitGet()
  ...
  val exchangeTransactionGen: Gen[ExchangeTransaction] = for {
    ...
    buyerMatcherFeeAssetId  <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, buyerAnotherAsset, Bdm)
    sellerMatcherFeeAssetId <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, sellerAnotherAsset, Bdm)
    r <- Gen.oneOf(
  ...
  def exchangeV2GeneratorP(
      ...
      buyMatcherFeeAssetId: Asset = Bdm,
      sellMatcherFeeAssetId: Asset = Bdm,
