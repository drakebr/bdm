...
inConfig(Compile)(
  Seq(
    ...
    includeFilter in PB.generate := new SimpleFileFilter((f: File) => f.getName.endsWith(".proto") && f.getParent.endsWith("bdm")),
...
inTask(assembly)(
  Seq(
    test := {},
    assemblyJarName := s"bdm-all-${version.value}.jar",
...
// Logback creates a "bdm.directory_UNDEFINED" without this option.
bashScriptExtraDefines ++= Seq(
  s"""addJava "-Dbdm.defaults.directory=/var/lib/${(Universal / normalizedName).value}"""",
  s"""addJava "-Dbdm.defaults.config.directory=/etc/${(Universal / normalizedName).value}""""
)

inConfig(Universal)(
  Seq(
    mappings += (baseDirectory.value / s"bdm-${network.value}.conf" -> "doc/bdm.conf.sample"),
    mappings := {
...
inConfig(Linux)(
  Seq(
    maintainer := "bdmplatform.com",
    packageSummary := "Bdm node",
    packageDescription := "Bdm node"
  ))
...
V.scalaPackage := "com.bdmplatform"

// Hack for https://youtrack.jetbrains.com/issue/SCL-15210

moduleName := s"bdm${network.value.packageSuffix}" // bdm-*.jar instead of node-*.jar
executableScriptName := moduleName.value // bin/bdm instead of bin/node
