...
import com.bdmplatform.account.{AddressOrAlias, AddressScheme, Alias}
...
import com.bdmplatform.state.BdmEnvironmentBenchmark._
import com.bdmplatform.state.bench.DataTestData
...
class BdmEnvironmentBenchmark {
  ...
  @Benchmark
  def accountBalanceOf_bdm_test(st: AccountBalanceOfBdmSt, bh: Blackhole): Unit = {
...
object BdmEnvironmentBenchmark {
  ...
  @State(Scope.Benchmark)
  class AccountBalanceOfBdmSt extends BaseSt {
  ...
  @State(Scope.Benchmark)
  class AccountBalanceOfAssetSt extends AccountBalanceOfBdmSt {
  ...
  class BaseSt {
    ...
    val environment: Environment[Id] = {
      ...
      val state            = new LevelDBWriter(db, portfolioChanges, bdmSettings.blockchainSettings, bdmSettings.dbSettings)
      new BdmEnvironment(
