...
object ExtensionPackaging extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[_]] =
    Seq(
      ...
      // To write files to Bdm NODE directory
  ...
  private def maintainerFix = inConfig(Linux)(
    Seq(
      ...
      packageSummary := s"Bdm node ${name.value}${network.value.packageSuffix} extension",
      packageDescription := s"Bdm node ${name.value}${network.value.packageSuffix} extension"
    ))
