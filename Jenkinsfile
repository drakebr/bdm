...
- Payload URL: https://<jenkins_web_address>/generic-webhook-trigger/invoke?token=bdmPipelineTriggerToken
...
- Definition: Pipeline script from SCM, SCM: Git, Repo: 'https://github.com/bdmplatform/Bdm.git',
...
ut = new utils()
scripts = new scripts()
def repoUrl = 'https://github.com/bdmplatform/Bdm.git'
def branch = false
...
def testTasks = [:]
def prInfo = [:]
def githubRepo = 'bdmplatform/Bdm'
def githubPersonalToken = 'bdm-github-token'
                        ...
                        node('integrationTests'){
                            stage('Integration Tests') {
                                ...
                                sh """
                                    ...
                                    {
                                        docker rm -f \$(docker ps -a | grep bdm | grep node | awk '{ print \$1 }') || true
