...
import com.bdmplatform.extensions.{Extension, Context => ExtensionContext}
import com.bdmplatform.settings.GRPCSettings
...
class GRPCServerExtension(context: ExtensionContext) extends Extension with ScorexLogging {
  ...
  override def start(): Unit = {
    val settings = context.settings.config.as[GRPCSettings]("bdm.grpc")
