...
import com.bdmplatform.lang.v1.repl.node.BlockchainUnavailableException
...
class ReplTest extends PropSpec with ScriptGen with Matchers with NoShrink {
  ...
  property("bdm context funcs absent") {
    val repl = Repl()
  ...
  property("url slash strip") {
    val url = "testnodes.bdmnodes.com"
  ...
  property("reconfigure") {
    ...
    val settings = NodeConnectionSettings("testnodes.bdmnodes.com", 'T'.toByte, address1)
  ...
  ignore("bdm context") {
    val settings = NodeConnectionSettings("testnodes.bdmnodes.com", 'T'.toByte, "3MpLKVSnWSY53bSNTECuGvESExzhV9ppcun")
    val repl = Repl(Some(settings))
    ...
    await(repl.execute(""" this.bdmBalance() """)).explicitGet() should fullyMatch regex "res12: Int = \\d+".r
  }
}
